...
static void VN_BANKED_CODE copy_data_ref_to_vram(uint16_t dest, const pce_editor_data_ref_t *ref, uint16_t word_stride, uint8_t cache_kind, uint16_t cache_asset_index)
{
#if !defined(__PCE_CD__)
    uint8_t i;
...
{
#if defined(__PCE_CD__)
    /* Keep the CD build on the conservative full-refresh path. The old fast path
       mapped bank133 and then waited/wrote SATB diff words while CD external IRQs
       could still arrive with slot4 not restored to bank130, which made long idle
       animation runs corrupt sprite SATB state. */
    return 0u;
#else
