...
/* Visual payload cache helper code is streamed into bank121 after System Card
   boot. Payload pages use low System Card RAM banks 112-119 and are raw-mapped
...
#define VN_VISUAL_CACHE_FIRST_BANK 112u
...
static uint8_t VN_BANKED_CODE2 visual_cache_copy_span_to_vram(uint16_t dest, uint8_t kind, uint16_t asset_index, unsigned long source_offset, uint16_t length)
{
    uint16_t remaining = length;
    uint16_t vram_dest = dest;
    while (remaining)
    {
        ...
        if (chunk > remaining) chunk = remaining;
        if ((uint16_t)(page_offset + chunk) > vn_visual_cache_size[slot]) return 0u;
        visual_cache_page_to_vram(vram_dest, slot, page_offset, chunk);
        vram_dest = (uint16_t)(vram_dest + ((chunk + 1u) / 2u));
        source_offset += (unsigned long)chunk;
        remaining = (uint16_t)(remaining - chunk);
    }
...
static uint8_t VN_BANKED_CODE2 visual_cache_bg_map_to_vram(uint16_t dest, uint16_t asset_index, const pce_editor_data_ref_t *ref, uint8_t width_tiles, uint8_t height_tiles)
{
    uint8_t row;
    const uint16_t row_bytes = (uint16_t)(width_tiles * 2u);
    if (!ref || !ref->size || !width_tiles || !height_tiles) return 0u;
    for (row = 0u; row < height_tiles; row++)
    {
        const unsigned long source_offset = (unsigned long)row * (unsigned long)VN_MAP_ROW_BYTES;
        if (!visual_cache_copy_span_to_vram(
            (uint16_t)(dest + ((uint16_t)row * VN_MAP_WIDTH)),
            VN_VISUAL_CACHE_KIND_BG_MAP,
            asset_index,
            source_offset,
