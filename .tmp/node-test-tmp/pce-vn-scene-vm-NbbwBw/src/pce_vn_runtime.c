...
#define VN_VISUAL_CACHE_OP_GET_BG_ASSET 8u
#define VN_VISUAL_CACHE_OP_GET_SPRITE_ASSET 9u
#define VN_VISUAL_CACHE_OP_GET_PSG_ASSET 10u
...
static const void *vn_visual_cache_result_ptr __attribute__((section(".bss")));
#endif
#endif
...
static const pce_editor_bg_asset_t *VN_VISUAL_CACHE_CODE vn_get_bg_asset_impl(uint16_t idx);
static const pce_editor_sprite_asset_t *VN_VISUAL_CACHE_CODE vn_get_sprite_asset_impl(uint16_t idx);
static const pce_editor_psg_asset_t *VN_VISUAL_CACHE_CODE vn_get_psg_asset_impl(uint16_t idx);
...
static void VN_RESIDENT_CODE load_visual_cache_code(void);
#endif
...
static uint8_t VN_VISUAL_CACHE_ENTRY_CODE visual_cache_entry(uint8_t op)
{
    ...
    if (visual_op == VN_VISUAL_CACHE_OP_GET_BG_ASSET)
    {
        vn_visual_cache_result_ptr = vn_get_bg_asset_impl(vn_visual_cache_arg_asset);
        return 0u;
    }
    if (visual_op == VN_VISUAL_CACHE_OP_GET_SPRITE_ASSET)
    {
        vn_visual_cache_result_ptr = vn_get_sprite_asset_impl(vn_visual_cache_arg_asset);
        return 0u;
    }
    if (visual_op == VN_VISUAL_CACHE_OP_GET_PSG_ASSET)
    {
        vn_visual_cache_result_ptr = vn_get_psg_asset_impl(vn_visual_cache_arg_asset);
        return 0u;
    }
...
{
    const pce_editor_bg_asset_t *result;
    load_visual_cache_code();
    vn_visual_cache_arg_asset = idx;
    (void)visual_cache_call(VN_VISUAL_CACHE_OP_GET_BG_ASSET);
    result = (const pce_editor_bg_asset_t *)vn_visual_cache_result_ptr;
    return result;
}
...
{
    const pce_editor_sprite_asset_t *result;
    load_visual_cache_code();
    vn_visual_cache_arg_asset = idx;
    (void)visual_cache_call(VN_VISUAL_CACHE_OP_GET_SPRITE_ASSET);
    result = (const pce_editor_sprite_asset_t *)vn_visual_cache_result_ptr;
    return result;
}
...
{
    const pce_editor_psg_asset_t *result;
    load_visual_cache_code();
    vn_visual_cache_arg_asset = idx;
    (void)visual_cache_call(VN_VISUAL_CACHE_OP_GET_PSG_ASSET);
    result = (const pce_editor_psg_asset_t *)vn_visual_cache_result_ptr;
    return result;
}
...
#else
#define vn_get_bg_asset_impl(idx) (&pce_editor_bg_assets[(idx)])
#define vn_get_sprite_asset_impl(idx) (&pce_editor_sprite_assets[(idx)])
#define vn_get_bg_asset(idx) vn_get_bg_asset_impl(idx)
#define vn_get_sprite_asset(idx) vn_get_sprite_asset_impl(idx)
#define vn_get_adpcm_asset(idx) (&pce_editor_adpcm_assets[(idx)])
#define vn_get_psg_asset_impl(idx) (&pce_editor_psg_assets[(idx)])
#define vn_get_psg_asset(idx) vn_get_psg_asset_impl(idx)
...
static void VN_RESIDENT_CODE restore_current_screen_palette(void)
{
    if (current_bg_index >= 0 && current_bg_palette_size)
    {
...
static uint8_t VN_RESIDENT_CODE load_psg_pattern_cd(const pce_editor_psg_asset_t *asset)
{
    uint8_t result;
    load_visual_cache_code();
...
static void VN_VISUAL_CACHE_CODE load_bg_cache_asset_impl(signed int bg_index, uint8_t tile_x, uint8_t tile_y)
{
    ...
    (void)tile_y;
    map_resident_data();
    bg = vn_get_bg_asset_impl((uint16_t)bg_index);
...
static void VN_VISUAL_CACHE_CODE load_sprite_pattern_cache_asset_impl(signed int sprite_index, uint8_t slot_index)
{
    ...
    (void)slot_index;
    map_resident_data();
    sprite = vn_get_sprite_asset_impl((uint16_t)sprite_index);
    SNAPSHOT_DATA_REF(sprite_patterns, sprite->patterns);
...
#else
static void VN_RESIDENT_CODE load_bg_cache_asset(signed int bg_index, uint8_t tile_x, uint8_t tile_y)
{
    (void)bg_index;
    (void)tile_x;
    (void)tile_y;
...
static void VN_RESIDENT_CODE clear_runtime_cache(uint8_t scope)
{
    load_visual_cache_code();
    vn_visual_cache_arg_scope = scope;
    (void)visual_cache_call(VN_VISUAL_CACHE_OP_CLEAR_RUNTIME_CACHE);
}
