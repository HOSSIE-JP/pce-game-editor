...
#define VN_VISUAL_CACHE_OP_SHOW_SPRITE_FRAME 15u
#define VN_VISUAL_CACHE_OP_REFRESH_SPRITE_PATTERNS 16u
#define VN_VISUAL_CACHE_OP_LOAD_SPRITE_PATTERN_CACHE 17u
...
static int16_t vn_visual_cache_arg_sprite_y __attribute__((section(".bss")));
static uint8_t vn_visual_cache_arg_frame __attribute__((section(".bss")));
static uint8_t vn_visual_cache_arg_flags __attribute__((section(".bss")));
...
static const pce_editor_sprite_draw_meta_t *vn_visual_cache_arg_draw_meta __attribute__((section(".bss")));
static const uint8_t *vn_visual_cache_arg_cell_map __attribute__((section(".bss")));
static const pce_vn_sprite_anim_t *vn_visual_cache_arg_animation __attribute__((section(".bss")));
#endif
#endif
...
static uint8_t VN_VISUAL_CACHE_CODE show_character_sprite_frame(uint8_t satb_index, const pce_editor_sprite_draw_meta_t *draw_meta, const uint8_t *cell_map, const pce_vn_sprite_anim_t *animation, uint8_t frame, int16_t x, int16_t y, uint8_t flags);
static uint8_t VN_VISUAL_CACHE_CODE refresh_scene_sprite_patterns_impl(void);
static void VN_VISUAL_CACHE_CODE load_sprite_pattern_cache_asset_impl(void);
...
static uint8_t VN_VISUAL_CACHE_ENTRY_CODE visual_cache_entry(uint8_t op)
{
    ...
    if (visual_op == VN_VISUAL_CACHE_OP_SHOW_SPRITE_FRAME)
    {
        return show_character_sprite_frame(
            (uint8_t)vn_visual_cache_arg_dest,
            vn_visual_cache_arg_draw_meta,
            vn_visual_cache_arg_cell_map,
            vn_visual_cache_arg_animation,
            vn_visual_cache_arg_frame,
            vn_visual_cache_arg_sprite_x,
            vn_visual_cache_arg_sprite_y,
            vn_visual_cache_arg_flags
        );
    }
    if (visual_op == VN_VISUAL_CACHE_OP_REFRESH_SPRITE_PATTERNS)
    {
        return refresh_scene_sprite_patterns_impl();
    }
    if (visual_op == VN_VISUAL_CACHE_OP_LOAD_SPRITE_PATTERN_CACHE)
    {
        load_sprite_pattern_cache_asset_impl();
        return 0u;
    }
    return 0u;
}
...
static uint8_t VN_BANKED_CODE call_overlay_show_character_sprite_frame(uint8_t satb_index, const pce_editor_sprite_draw_meta_t *draw_meta, const uint8_t *cell_map, const pce_vn_sprite_anim_t *animation, uint8_t frame, int16_t x, int16_t y, uint8_t flags)
{
    ...
    uint8_t irq;
    if (!vn_visual_cache_code_loaded) return 0u;
    vn_visual_cache_arg_dest = satb_index;
    vn_visual_cache_arg_draw_meta = draw_meta;
    vn_visual_cache_arg_cell_map = cell_map;
    vn_visual_cache_arg_animation = animation;
    vn_visual_cache_arg_frame = frame;
    vn_visual_cache_arg_sprite_x = x;
    vn_visual_cache_arg_sprite_y = y;
    vn_visual_cache_arg_flags = flags;
    irq = vn_vdc_irq_lock();
    written = visual_cache_call(VN_VISUAL_CACHE_OP_SHOW_SPRITE_FRAME);
    vn_vdc_irq_unlock(irq);
    return written;
#else
...
static uint8_t VN_BANKED_CODE refresh_scene_sprite_patterns(void)
{
    ...
    /* The animation fast path only rewrites cached SATB pattern/attr words.
       ...
       low-RAM visual code bank, then restore bank130 before re-enabling IRQs. */
    if (!vn_visual_cache_code_loaded) return 0u;
    irq = vn_vdc_irq_lock();
    result = visual_cache_call(VN_VISUAL_CACHE_OP_REFRESH_SPRITE_PATTERNS);
    vn_vdc_irq_unlock(irq);
    return result;
#else
    return refresh_scene_sprite_patterns_impl();
...
static void VN_VISUAL_CACHE_CODE load_sprite_pattern_cache_asset_impl(void)
{
    const signed int sprite_index = (signed int)vn_visual_cache_arg_sprite_x;
    const pce_editor_sprite_asset_t *sprite;
    pce_editor_data_ref_t sprite_patterns;
    if (sprite_index < 0) return;
    if ((unsigned int)sprite_index >= pce_editor_sprite_asset_count) return;
    (void)vn_visual_cache_arg_slot;
    map_resident_data();
...
static void VN_BANKED_CODE load_sprite_pattern_cache_asset(signed int sprite_index, uint8_t slot_index)
{
    load_visual_cache_code();
    if (!vn_visual_cache_code_loaded) return;
    vn_visual_cache_arg_sprite_x = (int16_t)sprite_index;
    vn_visual_cache_arg_slot = slot_index;
    (void)visual_cache_call(VN_VISUAL_CACHE_OP_LOAD_SPRITE_PATTERN_CACHE);
}
#else
