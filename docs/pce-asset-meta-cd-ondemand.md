...
- CD オンデマンドモードでは、consumer 側の配列添字参照が accessor 戻りポインタ経由になり、
  ...
  runtime-owned snapshot に落としてから使う。Kitahe の meta 強制ビルドではさらに
  `refresh_scene_sprite_patterns_impl()` を Path B overlay（bank133）へ逃がし、
  `cache_sprite_animation()` / `adpcm_voice_fits_buffer()` を bank130 へ寄せることで
  bank129=8079B, bank130=8155B, overlay=3775B に収めている。閾値を超えたのに 129/130 が
  溢れる場合は、まずこの consumer snapshot と bank129/130/133 の配置を確認する。
