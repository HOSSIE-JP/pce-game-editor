...
`stream: true` の ADPCM は `pce_cdb_adpcm_stream()` へ CD sector と sector count を渡して再生します。長尺音声向けなので import 時は ADPCM RAM の 16-bit size/address 制約で自動分割せず、1つの CD data file として保持します。streaming 中は CD data read を同時に使えないため、BG/sprite の表示データや別 ADPCM preload は streaming 開始前に済ませてください。streaming asset は preload command では読み込まず、audio/message の再生タイミングで開始します。
...
| ADPCM streaming | `adpcm.options.stream: true` は preload をスキップし、play 時に `pce_cdb_adpcm_stream()` で CD data file を直接再生する |
