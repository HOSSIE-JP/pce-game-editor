...
ADPCM は WAV から `assets/generated/<id>/adpcm.bin` へ変換されます。CD-ROM2 target では CD data file として配置され、runtime が ADPCM RAM へ読み込みます。生成される ADPCM は OKI/MSM5205 4-bit ADPCM の高位 nibble 先 (`msn-first`) です。古い低位 nibble 先の generated file は、source WAV が残っていれば build/source 生成時に自動再生成されます。WAV / MP3 の加工 UI は `pce-audio-converter` が提供し、trim、正規化、volume dB、fade in/out、波形 preview、seek、sample rate、mono/stereo を適用した WAV Data URL を `importAssetAudio()` に渡します。
...
現行 VN runtime の CD-DA 再生は、明示的な audio command がある場合だけ開始します。開始位置は `cdda-track.options.track`、終了位置は TOC から取得した次 track 開始 sector または lead-out です。`cdda-track.options.loop` に応じて `PCE_CDB_CDDA_PLAY_REPEAT` / `PCE_CDB_CDDA_PLAY_ONE_SHOT` を切り替えます。
...
現行 runtime は ADPCM の preload/cache 状態を `loaded_adpcm_valid` / `loaded_adpcm_index` で管理します。すでに同じ ADPCM が読み込まれている場合、preload は controller を reset/load しません。また ADPCM 再生中の preload は ADPCM RAM の再読み込みを避けるため、`audio` command の直後に背景表示などが scene preload を走らせても再生中の音声を壊しません。音声の確実な再生制御をしたい場合は、まず `audio` command または `message.voiceAssetId` を主 API として使い、`preload` は表示データの読み込みタイミング調整を主目的にしてください。
...
PCE-CD / Super CD-ROM2 build では、llvm-mos の既定 `.text` / `.rodata` は常駐 `ram_bank128` に配置されます。VN runtime は command interpreter と sprite refresh を `.ram_bank129`、font tiles と VN command/message/scene tables を `.ram_bank132` に明示配置し、bank128 に全 runtime/data を詰め込まない構成にしています。bank130-131 は例外的な小さい CPU-readable fallback data 用に残し、背景 tiles/map、sprite pattern、ADPCM 本体は `cd.dataFiles` から VRAM / ADPCM RAM へ直接転送します。詳細な割り当てと変更時の注意は `docs/pce-memory-bank-strategy.md` を参照してください。
...
CD-ROM2 VN runtime は `map_vram.bin` を `mapBase` から丸ごとVRAMへ置くblobとして扱いません。`map_vram.bin` は1行64タイルのソースとしてCDからsector単位でscratchへ読み、各行の `width_tiles` 分だけを `mapBase + row * 64` にコピーします。これにより、背景画像より広い画面領域の左右/上下余白は `clear_screen_map()` のblank tileのまま残り、CD map paddingの0 wordが古いVRAM tileを参照して縦縞になる事故を防ぎます。
...
CD-ROM2 runtime は、ADPCM RAM への `read_from_cd` / `read_from_ram` 後に BIOS status の busy bit が落ちるまで短く待ってから `pce_cdb_adpcm_play()` を呼びます。さらに再生中は scene preload が ADPCM controller を reset しないため、音声 command の直後に背景や sprite の preload が入っても再生中サンプルを破壊しません。
...
sequenceDiagram
  ...
  RT->>CDB: pce_cdb_cdda_play(track, same track end, loop ? REPEAT : ONE_SHOT)
  Scene->>RT: kind=cdda, action=stop
  RT->>CDB: pce_cdb_cdda_pause()
```

CD-DA は `cdda-track.options.track` を使って track 番号で再生されます。現行 runtime では track が 2 未満なら再生しません。再生時に CD TOC を読み、途中の track は次 track 開始 sector、最後の track は lead-out time を終了位置として渡すため、one-shot 再生が次 track へ続きません。
...
| ADPCM format | generated `adpcm.bin` は OKI/MSM5205 4-bit ADPCM の高位 nibble 先。古い generated file は source WAV があれば build/source 生成時に再生成する |
| ADPCM preload | `preload` は読み込みタイミング調整用。読み込み済み/再生中の ADPCM は reset/load せず、再生制御は `audio` command または `message.voiceAssetId` を主 API にする |
