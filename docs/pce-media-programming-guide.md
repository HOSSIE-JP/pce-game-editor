...
| `loop` | `false` | buffered 再生では repeat mode のまま runtime が停止 frame を管理し、true streaming では loop 再発行に使う |
| `stream` | `false` | `true` の場合は preload を行わない。**ただし ADPCM RAM に収まる音声は安定した buffered 経路で再生し**、`pce_cdb_adpcm_stream()` による真の CD streaming は ADPCM RAM に収まらない大きい asset のときだけ使う |
| `adpcmAddress` | `0..65535` | ADPCM RAM 上の読み込み先 address |
| `divider` | `0..15`, 通常は自動 | ADPCM hardware / `pce_cdb_adpcm_stream()` に渡す ADPCM 再生 rate code。未指定時だけ `sampleRate` から自動計算する |
...
`stream: true` の ADPCM は scene 入場時の内部 preload を行いません。**ただし direct-buffered 安全上限（`min(32767, 65536 - adpcmAddress)` bytes）に収まる音声は、`stream: true` でも安定した buffered 経路（`pce_cdb_adpcm_read_from_cd()` → direct ADPCM playback）で再生します。** buffered playback は System Card BIOS の `pce_cdb_adpcm_play()` を使わず、ADPCM read address / `0xffff` length / divider を直接 latch して repeat mode で開始します。通常は generated `play_frames` より数 frame 早く runtime が direct stop / loop restart します。CD unit IRQ と System Card pending latch は runtime 側で消し、BIOS の完了 IRQ path に戻さないことで、終端後の repeat wrap と PSG/VDC 停止を抑えます。ADPCM RAM への CD load は、読み込み sector 数に応じた settle wait を終えてから external IRQ を閉じます。`pce_cdb_adpcm_stream()` による真の CD streaming は、direct-buffered 安全上限を超える大きい asset のときだけ使います。これは真の streaming が System Card BIOS の非同期 external IRQ で CD から ADPCM ring buffer を供給し続ける方式で、VBlank/VDC を IRQ なしの VDC status polling で自前所有するこの runtime と衝突し、buffer underrun によるノイズ・隣接 asset セクタの読み込み（再生中に「別の音声」が混入）・CD エンジンのハングを起こすためです。真の streaming を使う大きい asset では、streaming 中に CD data read を同時に使えないため、BG/sprite の表示データや別 ADPCM 読み込みは streaming 開始前に済ませてください。streaming asset の loop は generated catalog の `play_frames` で再発行します。

通常表示中または PSG 再生中は、runtime が真の CD streaming を開始しません。長い message voice は `splitPolicy: "auto"` で direct-buffered 安全上限（既定 address では 32767 bytes）以下の part へ分けるか sample rate を下げ、buffered playback で再生してください。
...
CD ロードと PSG の同時再生について: CD 転送待ち（`cd_transfer_wait()`）の補償 tick は、**常駐パターン（`psg_pattern_banked == 0`）のときは settle wait 内に分散**して `service_psg_during_blocking_frames(1u)` を呼び、シーケンサをほぼ実時間で滑らかに進めます。これにより、スプライト／背景の CD ロード中に短い one-shot SFX が一括補償で一気に末尾まで早送り（→`stop_psg()`）され無音化する問題を防ぎます。CD ストリーミング中のパターン（bank134/135）は、per-tick の bank remap が CD DMA（bank132 宛て）と競合しないよう、従来どおり wait 後にまとめて補償します。raw BG/sprite の CD payload は sector read が終わるたびに CD-DA を再開してから `cd_transfer_scratch` → VRAM/BAT copy に入るため、タイル書き換え中まで CD-DA pause を引きずりません。PSG 再生中の VRAM 転送は約32 byte sliceごとに区切り、slice ごとに VBlank の経過を見て `service_psg_during_blocking_work()` で最大 1 tick 消費し、MPR 復帰を挟みます。slice 数だけ SONG が早送りされないよう、HuC6280 timer IRQ ではなく VDC status の polling と、`delay_frame()` が VBlank 待ちを終えた直後の明示 tick で cooperative に進めます。sprite SATB 全転送と pattern/attr 差分更新では、非再入の VDC 書き込み中には触らず、VDC ロック直前/直後で同じ PSG service を呼びます。通常フレームは main loop 先頭ではなく `delay_frame()` 内で VBlank 待ち直後に 1 tick 進めます。PSG が鳴っていない場合は最大 1 sector までまとめて転送します。
...
現行 runtime は ADPCM の cache 状態を `loaded_adpcm_valid` / `loaded_adpcm_index` で管理します。scene 入場時の内部 preload や実際の `audio` / `message.voiceAssetId` 再生時に必要な ADPCM を読み込み、すでに同じ ADPCM が読み込まれている場合は controller を reset/load しません。音声の確実な再生制御は `audio` command または `message.voiceAssetId` を主 API にしてください。
...
`scope` は `visual`（BG + Sprite）/ `bg` / `sprite` / `adpcm` / `all` です。`visual` は `preloaded_bg_valid`、`preloaded_scene_visual_valid`、sprite pattern の loaded flag を落とし、`adpcm` は `loaded_adpcm_valid` だけを落とします。`all` は visual + ADPCM に加えて message glyph cache を無効化します。いずれも現在の VRAM / BAT / SATB / ADPCM controller / CD-DA / PSG / scene pack / 変数には触れず、次に該当 asset を使う command で再ロードさせるための非破壊操作です。scene pack command record は既存 19 byte のままで、`type = PCE_VN_COMMAND_CACHE`、`flags = PCE_VN_CACHE_ACTION_CLEAR`、`arg0 = PCE_VN_CACHE_SCOPE_*`、その他 field は sentinel / 0 です。
...
buffered playback の停止は ADPCM hardware の PLAY bit を直接落とします。true streaming だけは System Card BIOS の `pce_cdb_adpcm_stream()` / `pce_cdb_adpcm_stop()` 経路を使います。

CD-ROM2 runtime は ADPCM RAM への読み込みに llvm-mos SDK の `pce_cdb_adpcm_read_from_cd()` / `pce_cdb_adpcm_read_from_ram()` を使います。direct-buffered 安全上限に収まる buffered asset は、その後の再生開始と停止を ADPCM hardware への direct latch / direct stop で行います。`adpcm.options.stream` が `true` の asset は scene 入場時の内部 preload をスキップしますが、安全上限に収まる場合は play 時に同じ buffered 経路（read_from_cd → direct playback）で再生し、`ad_stream_start` 相当の `pce_cdb_adpcm_stream()` による真の streaming は安全上限を超える大きい asset のときだけ使います。

ただし通常表示中または PSG 再生中の真の streaming 開始は拒否します。Geargrafx では System Card external IRQ が実 VDC R5 を blank 側へ戻し、PSG の cooperative service も止めるため、buffered playback に収まらない voice は安全のため鳴らしません。

ADPCM RAM への読み込みが成功した場合、RAM source は ADPCM controller の busy bit が落ちるまで待ち、CD source は読み込み sector 数に応じた `cd_transfer_wait()` を終えてから同じ busy bit を確認します。CD 側の sector 転送が残ったまま external IRQ を閉じると、Geargrafx では SCSI phase が `COMMAND`、`sectors_left` が残った状態で止まることがあるためです。buffered playback では direct latch で再生を開始します。再生開始時には直前の buffered ADPCM を direct stop してから再生し、true streaming のときだけ BIOS stop/reset 経路を使います。ADPCM の data size、address、divider、loop、stream、play_frames、CD sector は BIOS 呼び出し前に runtime-owned snapshot へ直接コピーし、CD / ADPCM BIOS helper のあとに MPR が変わっても bank128 の asset metadata を読み間違えないようにします。さらに再生中は scene 入場時の内部 preload が ADPCM controller を reset しないため、音声 command の直後に背景や sprite の読み込みが入っても再生中サンプルを破壊しません。真の ADPCM streaming（`pce_cdb_adpcm_stream()`）は direct-buffered 安全上限を超える `stream: true` asset だけが対象で、収まる音声は buffered 経路で再生します。非loop の buffered / streaming 再生は、再生開始後に `pce_cdb_adpcm_status()` の stopped bit を毎 frame 監視せず、generated catalog の `play_frames` で自然終了を管理します。buffered direct playback は hardware repeat mode の wrap より数 frame 早く PLAY を落とし、自然終了後に追加の `pce_cdb_adpcm_stop()` / `pce_cdb_adpcm_reset()` は発行しません。標準 EmulatorJS/WASM core では buffered ADPCM one-shot の完了IRQで CPU が止まることがあるため、runtime は ADPCM load / CD data read / true stream stop など BIOS 操作時だけ external IRQ を有効にし、非loop buffered 再生中は完了IRQをマスクします。streaming loop も同じ frame counter で再発行します。ADPCM BIOS call 直後の message advance edge が落ちることもあるため、ADPCM 再生開始後は次の joypad edge 判定を一度だけ初期化します。このとき現在押されている button を baseline にし、押しっぱなしの I/RUN を新規 edge として扱いません。ADPCM BIOS helper は表示状態や VDC timing register を変えることがあるため、runtime は ADPCM load/stream 後に VBlank へ寄せてから 256x224 の解像度、BG size、SATB start、scroll、R5 shadow を再適用します。表示中なら display を戻し、暗転中の内部 preload では意図した暗転を維持します。VN の audio command は buffered direct playback / `pce_cdb_adpcm_stream()` を開始したら待ち状態を返さず次の command へ進みます。ただし未読み込みの通常 ADPCM は、再生開始前の ADPCM RAM 読み込みだけ同期的に完了待ちします。CD-DA 再生中に ADPCM や画像/sprite の CD data file を読む場合は、CD drive を data read に戻すため runtime が CD-DA を pause します。
Generated C の `data_size` は `unsigned long` field として出力し、長尺 ADPCM でも llvm-mos の16bit `unsigned int` literal に丸められないようにします。ここが丸まると、streaming の再生長や CD sector count を誤り、音声の途中切れや過走につながります。
...
| ADPCM streaming | `adpcm.options.stream: true` は preload をスキップするが、direct-buffered 安全上限に収まる音声は buffered 経路で再生する。`pce_cdb_adpcm_stream()` による真の CD streaming は安全上限を超える大きい asset のときだけ使う |
