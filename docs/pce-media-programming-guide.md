...
明示 load は同じ `cache` command の `action: "load"` を使います。対象は1 commandにつき1 assetです。`scope: "adpcm"` はADPCM停止中だけADPCM RAMへ読み込みます。`scope: "bg"` / `"sprite"` は JSON / UI / scene pack 上は予約されていますが、標準 Super CD-ROM2 runtime では bank112-120 の visual payload cache を無効化しているため、VRAM/BAT/SATB、`sprite_slots[]`、現在の表示状態を変更しない no-op です。scene pack command record は19 byteのまま、`flags = PCE_VN_CACHE_ACTION_LOAD`、`arg0 = PCE_VN_CACHE_SCOPE_*`、`assetIndex` / `slot` / `x` / `y` に対象を格納します。
