...
PCE-CD / Super CD-ROM2 build では、llvm-mos の既定 `.text` / `.rodata` は常駐 `ram_bank128` に配置されます。VN runtime は command interpreter と sprite refresh を `.ram_bank129`、font tiles と VN command/message/scene tables を `.ram_bank132` に明示配置し、bank128 に全 runtime/data を詰め込まない構成にしています。
