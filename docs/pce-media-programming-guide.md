...
CD-ROM2 VN runtime の `background` command は同期 command です。既存 BG が表示中の `transition: "cut"` でも短い暗黙 palette fade out/in を挟み、BG の VRAM/BAT 転送と表示復帰が完了してから次 command へ進みます。明示 `fade` と同じく BG palette bank だけを段階変更し、メッセージ UI palette までは暗転させません。
...
`sprite` command も同期 command です。同一 slot に別 asset をロードする場合は、sprite layer を無効化して画面外 SATB を反映し、pattern VRAM 転送後に SATB と sprite layer を再有効化してから次 command へ進みます。VRAM 書き換え中の中間表示は見せません。同じ asset 内の目パチ・口パク frame 更新は pattern を再転送せず、既存 SATB layout の pattern word だけを差分更新します。
...
`voiceAssetId` に ADPCM が指定されている場合、VN source 生成時に ADPCM の再生長から文字送り速度を自動算出し、scene pack の `textSpeedFrames` に焼き込みます。runtime はゲーム中に再計算せず、この保存済み `text_speed_frames` をそのまま使います。これにより本文が最後まで表示されるタイミングと音声終了がほぼ同期します。`advanceMode: "button"` で typewriter をボタンスキップ（即時全文表示）した後、さらにボタンで次ページへ送ると、まだ再生中の ADPCM は `stop_adpcm_voice()` で停止します。ループ ADPCM や再生長を確認できない asset の場合は同期せず `textSpeedFrames` をそのまま使います。
