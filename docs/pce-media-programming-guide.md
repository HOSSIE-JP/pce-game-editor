...
| `divider` | `0..15`, 通常は自動 | `pce_cdb_adpcm_play()` / `pce_cdb_adpcm_stream()` に渡す ADPCM 再生 rate code。未指定時だけ `sampleRate` から自動計算する |
