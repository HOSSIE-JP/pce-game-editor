...
| `stream` | `false` | `true` の場合は preload を行わず、短尺でも CD data file を `pce_cdb_adpcm_stream()` で直接再生する |
...
`stream: true` の ADPCM は CD data file を `pce_cdb_adpcm_stream()` で直接 streaming 再生します。短尺で ADPCM RAM に収まる場合でも buffered 再生へ戻しません。streaming 中は CD data read を同時に使えないため、BG/sprite の表示データや別 ADPCM 読み込みは streaming 開始前に済ませてください。streaming asset は scene 入場時の内部 preload では読み込まず、audio/message の再生タイミングで開始し、loop は data size と sample rate から計算した frame counter で再発行します。
...
| ADPCM streaming | `adpcm.options.stream: true` は preload をスキップし、短尺でも `pce_cdb_adpcm_stream()` で CD data file を直接再生する |
