...
- **共通 VRAM copy**（`pce_editor_vram_copy()`、resident/noinline）= `write_map_words()` の BAT 行更新、message window clear、raw BG/map/font/sprite pattern の `pce_vdc_copy_to_vram()` を IRQ guard 付きで実行。message 開始時の `clear_window_cells()` は BAT と message strip 208 タイルを連続で触るため、ここが未ガードだとランダムな一瞬の BG/メッセージ破壊になる。
- **SATB 全アップロード**（`upload_sprite_table()`）= BG/シーン変更後のスプライト再表示。

これらは bank128 常駐（または常駐ラッパー）でガードを 1 か所に持つことで、満杯の bank129/130 へインライン展開させずに収めています。むしろ巨大な compositor 関数（`draw_message_*`）を**呼び出し元4箇所への重複インラインから常駐単一実体へ移す**ことで bank の空きが増えました（Phase 2 + BAT row guard 後の Kitahe 実測 bank130 は 6726B / 82.10%）。
...
- **未ガードの残り**: BG/Sprite の旧 RLE writer が古い generated data に残る場合は個別確認が必要。現行 generated visual payload は raw 化済みで、raw の CD blit / map row copy / BAT row write は `pce_editor_vram_copy()` の共通 guard を通る。
