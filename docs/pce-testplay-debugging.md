...
- **共通 VRAM copy**（`pce_editor_vram_copy()`、resident/noinline）= `write_map_words()` の BAT 行更新、message window clear、raw BG/map/font/sprite pattern の `pce_vdc_copy_to_vram()` を IRQ guard 付きで実行。message 開始時の `clear_window_cells()` は BAT と message strip 208 タイルを連続で触るため、ここが未ガード、または表示中にそのまま走るとランダムな一瞬の BG/メッセージ破壊になる。message 開始/全文 reveal は `begin_message_window_vram_update()` で display blank と `pending_display_enable` を立て、clear/先読み/初回 glyph/一括 glyph draw が終わってから `end_message_window_vram_update()` で戻す。
...
これらは bank128 常駐（または常駐ラッパー）でガードを 1 か所に持つことで、満杯の bank129/130 へインライン展開させずに収めています。巨大な compositor 関数（`draw_message_*`）は bank133 overlay へ逃がし、sprite 差分 refresh と message window blanking helper は bank130 に置いています（Phase 2 + VBlank/VDC/SATB/message-window hardening 後の Kitahe 実測: bank128=8087B(98.72%)、bank129=7963B(97.20%)、bank130=7545B(92.10%)、overlay=2260B/4096B）。
