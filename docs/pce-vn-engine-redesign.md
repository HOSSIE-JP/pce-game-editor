...
- `VN_PSG_CD_TRANSFER_COMPENSATION_FRAMES` は現在 `10u`。旧ドキュメントに記載された `24u` の一律バーストや `VN_CD_CHUNK_ESTIMATED_FRAMES` は解消済みで、補償は PSG 専用に即時適用する。CD helper 後の settle poll は `VN_CD_TRANSFER_SETTLE_POLL_ITERATIONS`（既定 `4096u`）へ短縮し、RAM 直読み payload は `VN_CD_RAM_READ_CHUNK_SECTORS`（既定 `2u`）までまとめる。visual payload cache の page miss-fill と visual-code 側 CD→VRAM fallback は、bank104〜119 のLRU pageを直接/一時 scratch として使い `VN_VISUAL_CACHE_CD_READ_CHUNK_SECTORS`（既定 `4u`）までまとめる。ADPCM RAM への CD read はその後 Phase 4 で bank122 direct SCSI async helper へ移行し、SCSI DATA IN を `IO_PCD_ADPCM_DATA` へ毎 frame 書く標準 path になった。旧 `pce_cdb_adpcm_read_from_cd` chunk 方式で見えていた seek 支配の PSG 凍結は標準 path から外れ、`VN_ADPCM_MESSAGE_READ_CHUNK_SECTORS` / `VN_ADPCM_PRELOAD_READ_CHUNK_SECTORS` は call-site 互換用に残るだけである。PSG 再生中の VRAM copy は `VN_VISUAL_VRAM_COPY_SLICE_BYTES`（既定 `16u`）で細かく service へ戻す。
...
- CD load chunking の 2026-07 追加作業では、CD ブラケット（CD-DA pause/resume、外部 IRQ、TIMER 所有）をフレーム跨ぎで開いたままにする「フル案B」は `vn_wait_next_vblank()` 内の `quiet_cd_unit_irqs()` と両立させない。標準 runtime は BIOS read 1 コマンドごとに bracket を閉じ、`PCE_VN_COMMAND_CACHE load` だけを協調状態機械化する。BG cache load は tiles/map の 2 substep、sprite/ADPCM/PSG cache load は 1 substep ごとに main loop へ戻り、各 substep 後に `wait_frames_remaining = 1u` で次フレームへ再入する。
