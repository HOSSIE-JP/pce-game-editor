...
- `VN_OVERLAY_LMA = 0x0184d040`（bank132 末尾 CPU 0xd040、良性 LMA。0xd040-0xdfff の window）
...
- **現状の上限は物理 bank133(8KB) ではなく、良性 LMA の置き場所（bank132 末尾 0xd040-0xdfff）**。CD 上の `overlay.bin` 予約は 4KB のまま固定し、実 `.vn_overlay` が LMA window に収まるかを `finalizeOverlayBlob()` で検査する。
