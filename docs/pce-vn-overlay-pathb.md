...
- **Path B = 未使用物理 bank133 にコードを置き、CD からブート時にロードして slot4 を bank130 と時分割する**機構。
  ...
  - **Phase B1（完了・本ドキュメント時点）**: 実コード（CD RLE 展開 `cd_rle_ref_to_vram` / `cd_rle_bg_map_ref_to_vram`）をオーバーレイへ退避し、**bank130 を 95% → 55%（7782 → 4494 bytes、約3.3KB）緩和**。Geargrafx で BG/sprite/入力の正常動作を実証済み。
- **重要な認識**: B1 で得た余白は「一度きりの約3.3KB ＋ 再利用可能な退避の仕組み」。常駐コード総枠（3バンク＝約24KB）は増えていない。bank128/129 は依然 ~94-96% でほぼ満杯。
...
| 退避済み関数 `cd_rle_ref_to_vram` / `cd_rle_bg_map_ref_to_vram` | 同上（`VN_OVERLAY_CODE` タグ）|
...
- `VN_OVERLAY_RESERVED_SECTORS = 2`（= 4096 bytes、CD/bank133 への固定予約）
...
→ overlay.bin 先頭バイトと一致すれば OK（cd_rle_ref_to_vram のエントリ "85 C6 86 C9 A6 02 ..." 等）
```
RLE 圧縮の BG/sprite が正常描画されれば、オーバーレイ経由の展開が動作している証拠（このプロジェクトの BG tiles / sprite patterns は `.rle`）。

## 7. 残課題と対応方針（Codex 向け）

### (A) オーバーレイ 4KB 上限の引き上げ
- **現状の上限は物理 bank133(8KB) ではなく、良性 LMA の置き場所（bank132 末尾 0xd000-0xdfff = 4KB）**。実コードが 0xcd8(3288) で残り約0.7KB。
...
- **候補アプローチ**（要・Geargrafx 実証）:
  1. LMA をより広い良性領域へ移す（例: bank132 のデータ配置を見直して連続 8KB を確保、または別の常駐バンクの空き末尾）。bank128/129 は満杯、bank130 は緩和後でも末尾約3.7KB しか空かない点に注意。
