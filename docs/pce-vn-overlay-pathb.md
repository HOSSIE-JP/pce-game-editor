...
> **関連するが別物**: `cache load bg/sprite` 用の visual payload RAM cache は bank133 Path B overlay ではなく bank120 / bank112-119 を使う実験枠でしたが、System Card boot 安定性を優先して標準 runtime では無効化しています。現行 build は `assets/generated/vn/visual_code.bin` を生成・ロードせず、BG/Sprite の VRAM 転送は常駐側の CD→scratch→VRAM chunked path で行います。
...
| bank120 宣言 `PCE_RAM_BANK_AT(120, 4)` | 同上。標準 runtime では `VN_ENABLE_VISUAL_PAYLOAD_CACHE 0` により無効。Path B overlay ではない |
...
| visual helper の CD ref / load addr | 標準 runtime では生成しない。再有効化時のみ `src/generated/vn.{c,h}` に `pce_vn_visual_code_data`、`PCE_VN_VISUAL_CODE_LOAD_ADDR` を出す |
...
- `VN_OVERLAY_LMA = 0x0184d078`（bank132 末尾 CPU 0xd078、良性 LMA。0xd078-0xdfff の window）。**この overlay 良性コピーは起動時に bank132 へロードされるが一切読まれない（実体は CD→bank133）。そこで `writeOverlayFragment` が固定ランタイムバッファ（`cd_transfer_scratch` 2KB ＋ `message_glyph_cache_masks` 1632B＝計~3.6KB、いずれも write-before-read）を `.ram_bank132_tail`(NOLOAD, CPU 0xd078) としてこの良性 window に重ね、無駄RAMを再利用する。結果 [0xc000, 0xd078) = 4216B が丸ごと「増える常駐メタデータ(cd_ref/cell_map/scene-pack ディレクトリ)」に空き、実測で常駐 metadata は 484B＝余裕約3732B（バッファ移設前は余裕約53B）。CD アセットが増えても約500件以上の cd_ref を追加できる。overlay 実測 ~3956B は [0xd078,0xe000) にバッファと重ねて収まる（finalizeOverlayBlob が overlay の収まりを検査）**
- `VN_OVERLAY_SECTION = '.vn_overlay'`
- `VN_VISUAL_CODE_LINK_ADDR = 0x01788000` / `VN_VISUAL_CODE_RESERVED_SECTORS = 4` / `VN_VISUAL_CODE_SECTION = '.vn_visual_code'` は bank120 の visual cache helper code 用に残す予約定数です。標準 runtime では `VN_ENABLE_VISUAL_PAYLOAD_CACHE 0` のため section / blob / CD ref は生成されません。再有効化する場合は `.vn_visual_code.entry` を先頭に固定して CPU 0x8000 へ dispatch し、出力 blob を `assets/generated/vn/visual_code.bin` に pad して main ELF から section 本体を除去する設計です。
