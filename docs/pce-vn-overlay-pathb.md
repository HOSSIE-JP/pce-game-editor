...
> **関連するが別物**: `cache load bg/sprite` 用の visual payload RAM cache は bank133 Path B overlay ではなく低位 System Card RAM を使う実験枠でしたが、System Card boot 安定性と Geargrafx の CD BIOS `DATA IN` 停止回避を優先して標準 runtime では無効化しています。現行 build は `assets/generated/vn/visual_code.bin` を生成・ロードせず、BG/Sprite の VRAM 転送は常駐側の CD→scratch→VRAM chunked path で行います。
...
| visual cache 用低位 RAM 宣言 | 同上。標準 runtime では `VN_ENABLE_VISUAL_PAYLOAD_CACHE 0` により無効。Path B overlay ではない |
