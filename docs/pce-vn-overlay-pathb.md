...
- `VN_OVERLAY_LMA = 0x0184d200`（bank132 末尾 CPU 0xd200、良性 LMA。0xd000-0xd1ff は resident metadata の余白）
...
- **現状の上限は物理 bank133(8KB) ではなく、良性 LMA の置き場所（bank132 末尾 0xd200-0xdfff = 3.5KB）**。Phase 2 + VBlank/VDC/SATB/message-window hardening 後の Kitahe 実測は約2260Bで、残りは約1320B。CD 上の `overlay.bin` 予約は 4KB のまま固定し、実 `.vn_overlay` が LMA window に収まるかを `finalizeOverlayBlob()` で検査する。
