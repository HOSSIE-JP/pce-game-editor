...
- `VN_OVERLAY_LMA = 0x0184d078`（bank132 末尾 CPU 0xd078、良性 LMA。0xd078-0xdfff の window）。**以前は 0xd040。CD ストリームする BG/sprite/scene が多い大規模 VN で常駐データ（cd_ref + sprite cell_map）が 0xc000-0xd040(4160B) 予算を数バイト超えて overlay LMA と衝突したため、固定サイズの overlay が使う tail 上端の余りを常駐へ返す形で divider を 0xd078(常駐 4216B) へ引き上げた。overlay 実測 ~3955B は [0xd078,0xe000) に約21B 余裕を残して収まる**
