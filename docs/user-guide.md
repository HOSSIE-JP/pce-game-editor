...
- **PSG（Sound タブの PSG）は `新規` で空のパターンを作るほか、`取込` で既存の VGM / VGZ / MIDI ファイルを登録**できます（拡張子で自動判別）。取込ダイアログで ID・Name・BPM・Type を指定すると、BPM から決まる 16 分音符グリッド（最大 256 ステップ）へ量子化し、psg-song / psg-sfx アセットとして登録します。長い曲は先頭 256 ステップで切り詰められ、警告が出ます。
  - **VGM / VGZ**: PC Engine（HuC6280）PSG レジスタ書き込みをそのまま量子化します。Type「自動」はループ情報で song / SFX を判定。波形・LFO・ノイズ・DDA は音程として近似されます。
  - **MIDI（.mid / .midi）**: 完全再現はできないため近似します。和音は **6 ボイスへ削減**（同時発音が 6 を超えると音程の高い音＝メロディを優先して残します）、**音程→period**、**ベロシティ→volume**、**ドラム（10ch）→ PSG ノイズ（ch4/5）** に変換します。**ピッチベンド／コントロールチェンジ／プログラムチェンジは再現されません**。BPM を空欄にすると MIDI のテンポを使います（テンポ変化が複数ある曲は最初のテンポでグリッドを固定し、警告を表示）。Type「自動」は曲（song・ループ）として登録します。
