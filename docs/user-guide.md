...
組み込みプラグイン **PCE VN GB Studio出力 v1.4.0** を有効にすると、Novel toolbarへ`GB Studio出力`が表示されます。CD-ROM2 / HuCARDのどちらもVN v2 `commands`を入力にできます。最初にGB Studio 4.3.1または4.3.2の実行file、空の出力folder、font、project全体の立ち絵modeを選択し、`Preflight`を実行してください。立ち絵modeの既定は「背景焼き込み」で、必要なら最大2人の「sprite actor」へ変更できます。engineは`4.3.0-e1`に固定され、違う版では停止します。fontの既定・推奨は組み込みMisaki Gothic 8x8です。BDF/TTF/OTF/TTCも選択でき、成功時にproject内へportable copyを保存します。
...
v1.4.0では`sprite`、`spritemove`、`spritetext`、fadeIn/out、flash、blank、shakeを変換します。背景modeは論理4slotを画面比率と重なり順で背景へ合成し、sync/async移動とanimationをtile timelineで再生します。actor modeは40×48 bustとA/B枠循環を使い、最大2人、合計40 OBJ・1走査線10 OBJが上限です。超過時は自動縮小や自動mode切替をせずpreflight errorになります。通常背景遷移は立ち絵を継承し、event stillは消去、blankは論理slotを保持したまま非表示にします。SpriteTextは同じPCE元scene内の内部segment間では維持し、別の元sceneへの遷移または同sceneへの明示jump/choice再入場では全slotを消去します。タイトル／シナリオ選択sceneでは黒文字へ変換し、「← シナリオ選択 →」を通常の比率変換位置から8px下げてtitle artworkとの間に余白を置きます。SpriteText内容・座標・色、立ち絵最終状態、本文・分岐・状態・BGMは省略できません。blink縮退やloop frame削減など許可された属性省略は個別に表示され、`visualOmissionsConfirmed`が必要です。時間・色・非原子的tile更新などの近似は`warningsAcknowledged`が必要です。inspection入力または監査hashが変わった場合は再確認してください。
...
`生成＋静的検証`はgenerator-owned GB Studio projectを生成して全参照を静的検査します。`生成＋公式build成果物必須`はさらに隔離profileの選択版でROM/Web Exportを実行し、compiler warning 0、ROM/Web内ROM hash一致、ROM CGB flag `0x80`を必須にします。出力先は空folderか同じsourceから作ったmanifest付きfolderだけを使えます。再生成時はowned fileだけをbackup・更新し、任意の既存GB Studio projectへmergeしません。

設定はPCE projectの`assets/pce-vn-gb-studio-export.json`へformat v1のまま`music` / `backgrounds` / `portraitRenderMode` / `sprite[assetId]`として保存します。`build/qa/control-flow-audit.json`には全source commandの一意な消費、処理区分、GBC/DMG event ID、到達性、背景/visual特殊化を保存し、`build/qa/visual-audit.json`にはcommand、asset、actor pair、timeline、tileset、OBJ/tile数、時間、fidelity、source/preview/export hashを保存します。完全SFX、4.3.1/4.3.2以外、既存project merge、全分岐自動playthroughなどPhase 4～6は[pce-vn-gb-studio-exporter.md](pce-vn-gb-studio-exporter.md)を参照してください。
