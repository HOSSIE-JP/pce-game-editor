...
CD-DAごとに登録済み`psg-song`またはGB Studio対応4ch ProTracker MODを指定します。独立したADPCM/PSG SFXはtone代替または明示省略を選びます。`message.voiceAssetId`は個別指定を要求せず、話者名から決定した安定frequencyのtext toneへ自動置換され、全件が`build/qa/conversion-audit.json`へ残ります。sprite/spritemoveと一部の視覚effectは列挙内容を確認した場合だけ省略できます。分岐・状態・本文・BGM、壊れた参照、未知commandは省略できません。targetなしasync Inputは、直後のWaitの早送り、または後続sync Inputの通常継続へ接続できる場合に変換されます。後続sync Inputと組になるscenario selectorでは全入力をscene-local callbackへ変換し、`EVENT_IDLE`で待機するためA/STARTと方向入力が互いをblockしません。どの経路でも遷移前に同groupのcallbackをすべて解除します。Message/Choice直前はGB Studio標準`EVENT_SET_DIALOGUE_FRAME`で既定frameを再転送し、独自native pluginは生成しません。各font pageにはGB Studio compiler用のPNG隣接`.json`も自動生成されます。古い出力にこのfileがない場合は、修正版でprojectを再生成してからROM/Webを再buildしてください。warningがある場合は曲別drop/transpose、低階調画像、視覚省略を確認してcheckboxをONにします。
