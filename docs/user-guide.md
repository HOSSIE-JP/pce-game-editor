...
v1.4.0では`sprite`、`spritemove`、`spritetext`、fadeIn/out、flash、blank、shakeを変換します。背景modeは論理4slotを画面比率と重なり順で背景へ合成し、sync/async移動とanimationをtile timelineで再生します。actor modeは40×48 bustとA/B枠循環を使い、最大2人、合計40 OBJ・1走査線10 OBJが上限です。超過時は自動縮小や自動mode切替をせずpreflight errorになります。通常背景遷移は立ち絵を継承し、event stillは消去、blankは論理slotを保持したまま非表示にします。SpriteText内容・座標・色、立ち絵最終状態、本文・分岐・状態・BGMは省略できません。blink縮退やloop frame削減など許可された属性省略は個別に表示され、`visualOmissionsConfirmed`が必要です。時間・色・非原子的tile更新などの近似は`warningsAcknowledged`が必要です。inspection入力または監査hashが変わった場合は再確認してください。
