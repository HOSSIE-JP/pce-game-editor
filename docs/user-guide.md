...
- **Slot（0〜3）**: 立ち絵を表示する 4 枚のスロットのどれを使うか。スロットごとに 1 枚の立ち絵を保持し、同じスロットへ別の `Sprite` コマンドを置くと差し替え（または非表示）になります。`Message` の **Mouth slot** はここで表示したスロット番号を指すので、口パクさせたい立ち絵はあらかじめ目的のスロットへ出しておきます。
...
- **Move（0〜255 フレーム）**: 立ち絵を新しい X/Y へ移動させるときのスライド時間（フレーム数）です。**同じスロットにすでに立ち絵が表示されている状態**で位置を変えた場合だけ有効で、その間に直前位置から指定座標へ補間移動します。初めて表示するときや非表示→表示の切り替えでは移動補間は行われず、即座に指定座標へ配置されます（`0` は常に即時配置）。
