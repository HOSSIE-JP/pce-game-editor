...
CD-DAごとに登録済み`psg-song`またはGB Studio対応4ch ProTracker MODを指定します。独立したADPCM/PSG SFXはtone代替または明示省略を選びます。`message.voiceAssetId`は個別指定を要求せず、話者名から決定した安定frequencyのtext toneへ自動置換され、全件が`build/qa/conversion-audit.json`へ残ります。`BGM調整`では曲catalogからtempo 50～200%とPCE 6chそれぞれのtarget、instrument、volume 0～200%、transpose -24～+24、priorityを指定できます。Source A / GB近似 BをWebAudioで再生・停止・loop確認できますが、hUGE完全再現ではないため最終判定は公式ROMで行ってください。`画像調整`では背景catalogからbrightness -100～+100、saturation 0～200%、GBC/DMG独立ditherを指定し、原画と変換後をGBC/DMG切替でpreviewできます。補正とditherはartwork領域だけに適用し、dialogue matteは変更しません。previewは選択assetだけを遅延生成し、古い応答を破棄します。調整値はmodalをキャンセルした場合は保存されず、正式export成功時だけsidecarへ保存されます。Phase 3対象のsprite/spritemove/fade/flash/blank等は凍結中で、列挙内容を確認した場合だけ省略できます。分岐・状態・本文・BGM、壊れた参照、未知command、正規化時に消えたcommandは省略できません。2～4択は必要なprojectだけへ生成する公式Script Event Plugin `plugins/pce-vn-control`で表示します。長いlabelは16セル単位で欠落なく折り返し、`defaultIndex`の初期カーソル、Bキャンセル無効、重複valueと各targetを保持します。native Cやengine overrideは生成しません。このcustom eventはGB Studio 4.3.1/4.3.2のcompiler helperへ固定しているため、別versionではpreflight errorです。PCEでsceneをまたいで保持される背景はCFGで伝播し、背景なし分岐sceneへ継承します。異なる背景が同じblockへ合流する場合はGB Studio sceneを背景別に特殊化します。comment/cacheだけの断片は到達不能warningにしません。warningがある場合は曲別drop/conflict、低階調画像、Phase 3視覚省略、本当に実行命令を持つ到達不能blockを確認してcheckboxをONにします。
選択肢は全labelの折返し後合計が画面上限16行を超えると`GBVN_CHOICE_MENU_OVERFLOW`で停止し、欠落や画面外表示を許しません。

`生成＋静的検証`はgenerator-owned GB Studio projectだけを生成します。`生成＋公式build成果物必須`はさらに隔離profileの選択版で通常のROM/Web Exportを実行し、warning 0件、ROM CGB flag `0x80`、Web `index.html`を必須にします。出力先は空folderか同じsourceから作ったmanifest付きfolderだけを使えます。再生成時はowned fileだけをbackup・更新し、任意の既存GB Studio projectへmergeしません。設定はPCE projectの`assets/pce-vn-gb-studio-export.json`（format v1の`music` / `backgrounds`）、所有権は出力rootの`pce-vn-gb-studio-export.manifest.json`、背景/BGM/変換監査は`build/qa/`へ保存されます。`build/qa/control-flow-audit.json`には全source commandの一意な消費、処理区分、GBC/DMG event ID、到達不能block、join、loopに加え、origin block、entry/effective background、特殊化scene IDを保存します。manifestには調整設定、source/normalized/output hashも記録します。立ち絵、複数portrait、fade/flash/blank、完全SFX、4.3系以外のGB Studio版、既存project merge、全分岐自動playthroughなどPhase 3～6は[pce-vn-gb-studio-exporter.md](pce-vn-gb-studio-exporter.md)を参照してください。
