...
`Novel` プラグインの `スクリプト` タブは VN シーンをコマンド単位で編集します。Scenes 一覧はドラッグ＆ドロップで順番を入れ替えられます。シーン名はヘッダの Name で編集でき、`chapter/opening` のように `/` で区切ると Scenes 一覧ではグループ見出しと leaf 名に分けて表示します。`Jump` などの参照は安定した scene `id` を使うため、名前を変えても遷移先は壊れません。中央のコマンド一覧では、各コマンド行の右側にアイコンボタンが並びます。
...
- **ボタンでのウェイトスキップ**: `Advance` が `button` のとき、表示途中にボタンを押すと、表示済みの文字は消さずに残りの本文だけを追加描画してページ送り待ちにできます。さらにボタンを押して次ページへ送ると、まだ鳴っている ADPCM ボイスは停止します。

`Audio` コマンドの **Kind** には `CD-DA` / `ADPCM` に加えて **`PSG`** を選べます。`PSG` を選ぶと PSG 音源アセット（song / SFX）を再生でき、**基準 ch**（0〜5）で再生を始めるチャンネルを指定できます（song はループ、SFX は一度だけ）。`stop` で停止します。PSG song の大きい pattern は再生開始時に RAM bank134/135 へ読み込まれるため、背景・スプライトなどの CD ロード中も PSG 自体はドライブを占有しません。runtime は CD 転送待ち、CD→VRAM 転送、fade の同期処理中にも補償 tick で PSG シーケンサを進め、BG 切替で曲が止まったり大きく遅くなったりしにくいようにしています（完全な割り込み駆動ではないため、非常に長い同期処理ではタイミングが粗くなる場合があります）。CD-DA 再生中にシーン変更や背景・スプライトの CD ロードが入る場合、ロード中は短く一時停止し、ロード後に同じトラックを再開します。Loop 有効の CD-DA は BIOS repeat で継続し、runtime が周期的に再生命令を出し直さないため、通常再生中の定期的な音切れを避けます。ただし CD から直接ストリーミングする ADPCM はドライブを使い続けるため、CD-DA とは同時継続できません。
...
- **PSG（Sound タブの PSG）は `新規` で空のパターンを作るほか、`取込` で既存の VGM / VGZ / MIDI ファイルを登録**できます（拡張子で自動判別）。取込ダイアログで ID・Name・BPM・Type を指定すると、BPM から決まる 16 分音符グリッド（最大 4096 ステップ / pattern event 最大 2048）へ量子化し、psg-song / psg-sfx アセットとして登録します。PSG タブの **再生/停止** はトグル式の WebAudio square/noise 疑似プレビューで、音程・リズム・noise の有無を確認するためのものです（実機の PSG 波形やミキサー特性とは完全一致しません）。PSG 一覧の `×` で選択したアセットを削除できます。
  - **VGM / VGZ**: PC Engine（HuC6280）PSG レジスタ書き込みをそのまま量子化します。Type「自動」はループ情報で song / SFX を判定。波形・LFO・ノイズ・DDA は音程として近似されます。
