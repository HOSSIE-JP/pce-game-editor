...
組み込みプラグイン **PCE VN プロジェクト結合** を有効にすると、CD-ROM2 projectのNovel toolbarへ`プロジェクト結合`が表示されます。project群のroot folderを1つ指定し、再帰探索された候補を左側のcheckboxで選択、右側で統合順を上下移動します。検索中の有効候補を一括選択でき、非CD、必須file不足、selector/SpriteText衝突、結合済み出力は理由付きで選択不可になります。現在projectは初期選択されますが解除可能です。初期出力先はroot、出力名は`<root名>_merged`です。各開始sceneの`NEXT_SCR` / `PREV_SCR`は選択順の輪へ接続され、同じ順で中央下部に`(1/N)`〜`(N/N)`が追加されます。現在projectが選択されている場合だけ検査前に未保存内容を保存し、生成後もactive projectは切り替わりません。CLIは`npm run merge:vn -- --output <出力先> [--title <タイトル>] [--dry-run] [--replace] <project1> <project2> ...`のままで、番号生成はtoolbarと共通です。詳細は [PCE VN 複数プロジェクト結合](pce-vn-project-merger.md) を参照してください。

### GB Studio出力

現行の利用先は、Windows x64向けスタンドアロン **PCE2GB Novel Game Converter**（`C:\homebrew\projects\pce-2-gb-novelgame-converter`）です。変換対象はdiskへ保存済みの`project.json`、`assets/pce-assets.json`、`assets/pce-vn-scenes.json`であり、PCE Game Editor上の未保存編集は参照しません。先にPCE projectを保存してからスタンドアロンで選択してください。

以下は移管parity検査のため一時保持している旧組み込み **PCE VN GB Studio出力 v1.5.0** の操作記録です。移管の公式build/runtime/Portable write監査が完了するまでは旧実装を削除しませんが、新規作業の実装元にはしません。旧プラグインを有効にするとNovel toolbarへ`GB Studio出力`が表示されます。CD-ROM2 / HuCARDのどちらもVN v2 `commands`を入力にできます。最初にGB Studio 4.3.1または4.3.2の実行file、空の出力folder、出力target、font、project全体の立ち絵modeを選択し、`Preflight`を実行してください。出力targetは`GB/GBC両対応（Color + Monochrome）`、`GBC専用（Color）`、`GB専用（Monochrome）`から選べます。専用targetは不要な反対機種のscene・背景・visual resource graphを生成しないため、両対応版の二重graphを持ちません。立ち絵modeの既定は「背景焼き込み」で、必要なら最大2人の「sprite actor」へ変更できます。engineは`4.3.0-e1`に固定され、違う版では停止します。fontの既定・推奨は組み込みMisaki Gothic 8x8です。BDF/TTF/OTF/TTCも選択でき、成功時にproject内へportable copyを保存します。
Windowsでは展開folder直下の`gb-studio.exe`を指定します。検出器が同階層の`resources/app.asar`をElectron対応の方法で検査するため、`app.asar`自体を選ぶ必要はありません。

CD-DAごとに登録済み`psg-song`またはGB Studio対応4ch ProTracker MODを指定します。独立したADPCM/PSG SFXはtone代替または明示省略を選びます。`message.voiceAssetId`は話者別text toneへ自動置換されます。`BGM調整`ではtempo 50～200%とPCE 6chそれぞれのtarget、instrument、volume、transpose、priorityをSource A / GB近似 BのWebAudio preview付きで調整できます。`画像調整`では背景別brightness、saturation、GBC/DMG独立ditherを原画/変換後で確認できます。WebAudioはhUGE完全再現ではないため、最終判定は公式ROMで行ってください。

`立ち絵調整`ではasset catalogと使用sceneを選び、source/GBC/DMG、animation再生・停止・loop、共通crop、scale 25～200%、offset、brightness、saturation、GBC/DMG別ditherを調整します。設定のcopy/paste、asset reset、全体resetが使えます。actor modeの指標はOBJ合計と走査線OBJ、背景modeの指標はkeyframe/tile予算です。previewは200ms debounceとgeneration番号で古い応答を破棄し、modalのcancelでは保存しません。source hashが変わった設定は維持したままstale warningを出すため、再previewしてください。

v1.5.0ではPhase 3の`sprite`、`spritemove`、`spritetext`、fadeIn/out、flash、blank、shakeを維持します。背景modeは論理4slotを画面比率と重なり順で背景へ合成し、sync/async移動とanimationをtile timelineで再生します。actor modeは40×48 bustとA/B枠循環を使い、最大2人、合計40 OBJ・1走査線10 OBJが上限です。超過時は自動縮小や自動mode切替をせずpreflight errorになります。通常背景遷移は立ち絵を継承し、event stillは消去、blankは論理slotを保持したまま非表示にします。SpriteTextは同じPCE元scene内の内部segment間では維持し、別の元sceneへの遷移または同sceneへの明示jump/choice再入場では全slotを消去します。タイトル／シナリオ選択sceneでは黒文字へ変換し、prompt・scenario title・counterをY=104/120/136へ16px間隔で配置します。GBCは画像用palette 0～6とは別にpalette 7の`default-ui`をSpriteText tileへ割り当てるため、背景ごとの量子化paletteに文字色が引きずられません。SpriteText内容・座標・色、立ち絵最終状態、本文・分岐・状態・BGMは省略できません。blink縮退やloop frame削減など許可された属性省略は個別に表示され、`visualOmissionsConfirmed`が必要です。時間・色・非原子的tile更新などの近似は`warningsAcknowledged`が必要です。inspection入力または監査hashが変わった場合は再確認してください。

2～4択は必要なprojectだけへ生成する公式Script Event Plugin `plugins/pce-vn-control`で表示します。長いlabel、`defaultIndex`、Bキャンセル無効、重複valueを保持します。targetなしchoiceはvalueを保存して後続switchへfallthroughできます。native Cやengine overrideは生成しません。PCEの持続背景とvisual stateはCFGで伝播し、異なる状態が同じblockへ合流する場合はGB Studio sceneをstable visual-state hash別に特殊化します。comment/cacheだけの断片は到達不能warningにしません。
選択肢は全labelの折返し後合計が画面上限16行を超えると`GBVN_CHOICE_MENU_OVERFLOW`で停止し、欠落や画面外表示を許しません。

`生成＋静的検証`はgenerator-owned GB Studio projectを生成して全参照を静的検査します。`生成＋公式build成果物必須`はさらに隔離profileの選択版でROM/Web Exportを実行し、compiler warning 0、ROM/Web内ROM hash一致、ROM CGB flagが選択target（両対応`0x80`、GBC専用`0xC0`、GB専用`0x00`）と一致することを必須にします。出力先は空folderか同じsourceから作ったmanifest付きfolderだけを使えます。再生成時はowned fileだけをbackup・更新し、任意の既存GB Studio projectへmergeしません。

設定はPCE projectの`assets/pce-vn-gb-studio-export.json`へformat v1のまま`targetMode` / `music` / `backgrounds` / `portraitRenderMode` / `sprite[assetId]`として保存します。`targetMode`は`dual` / `gbc` / `gb`です。`build/qa/control-flow-audit.json`には全source commandの一意な消費、処理区分、有効targetのevent ID、到達性、背景/visual特殊化を保存し、`build/qa/visual-audit.json`にはcommand、asset、actor pair、timeline、tileset、OBJ/tile数、時間、fidelity、source/preview/export hashを保存します。完全SFX、4.3.1/4.3.2以外、既存project merge、全分岐自動playthroughなどPhase 4～6は[pce-vn-gb-studio-exporter.md](pce-vn-gb-studio-exporter.md)を参照してください。
...
組み込みプラグイン **NVプロジェクトのGodotエクスポート** が有効な場合だけ、Novel画面上部に`Godot出力`を表示します。現在のGUI/JSON編集状態からGodotネイティブプレイヤーへ取り込む`*.pcevn.zip`を作成し、ZIP保存が成功した後、その出力に使った同じscene snapshotを`assets/pce-vn-scenes.json`へ保存します。CD-ROM2 / HuCARD VNの両方で利用でき、Sceneと参照中のアセットだけを収録します。BG/Spriteは、画像importのresize/crop確定後・PCE減色前に保存した高画質PNGを`visual.hd`、最終PCE palette/tile/pattern/cell mapから再構成したindexed PNGを`visual.pce`として二系統収録します。標準importは高画質PNGを`assets/images-hd/`または`assets/sprites-hd/`へ保持します。旧assetで減色前PNGがない場合は現行sourceへfallbackし、`stats.visualHighQualityFallbackAssets`と出力ログで件数を確認できます。manifestは`pce-vn-godot-package` version 3、`data/assets.json`はversion 2で、Godot runtimeは既定`hd`と`pce`をワンボタンで全画面切替できます。CD-DA/ADPCMはWAV/OGG/MP3の再生可能source（CD-DAは生成WAVを優先）、PSGはpattern metadataを使います。WAVはZIP作成時にOgg Vorbis（VBR quality 4、元のsample rate/channel数を維持）へ変換し、既存OGG/MP3は再encodeしません。Novelの`フォント`タブで選択中のproject font（`assets/pce-font.json`の`fontPath`）がTTF/OTF/WOFF/WOFF2なら同梱し、未選択font libraryの先頭fileへfallbackしません。System Card、IPL、ROM/CUE/ISO、PCE向けraw binaryは含みません。同じeditor projectから再出力したpackageはGodot側で更新として扱われ、別projectはライブラリに併存します。詳細なJSON契約とruntime切替条件は [Godot VN package exporter](pce-vn-godot-exporter.md) を参照してください。プラグインをOFFにするとボタンも非表示になります。
...
CD-ROM2 VN buildでは、同一ビルドから参照できる正式上限をADPCM 2048件、BG 8192件、Sprite 1024件、Sprite Animation合計1024件、System Card PSG package variant 1024件、ゲーム用CD-DA 97本（Track 3〜99）とします。PSG variantは`assetId`と再生channelの組ごとに1件で、参照PSG source asset自体も1024件までです。これはCD容量、1 assetのサイズ、Spriteの同時表示4 slotなどとは別の上限です。CD-DA以外の未参照assetは数えません。BG / Sprite / ADPCM / CD-DA metadata、Sprite Animation metadata、System Card PSG metadata/package、scene pack、BG/Sprite/ADPCM payloadは2048-byte境界で`vn_payload.bin`へ集約し、論理sector aliasで読み込みます。警告音声とゲーム用CD-DA音声trackはpack対象外です。詳しい数え方と個別制約は[pce-vn-large-project-limits.md](pce-vn-large-project-limits.md)を参照してください。
...
- **シーン遷移**: PCE実機と同じく、シーンが変わると全SpriteText slotを消去します。通常シーン間の遷移でも前シーンの文字は引き継ぎません。
