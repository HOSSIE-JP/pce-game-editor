...
`Image` の `BG` では PCE 背景画像を、`Sprites` では PCE sprite sheet を編集します。画像の `Palette bank`、出力幅/高さ、`Transparent index` は import 時に決める変換条件です。Compression は既定で `Auto` になり、CD-ROM2 では BG tile/map や sprite pattern の RLE sidecar が raw より小さい場合だけ CD 上で使われます。変換後に生成済み tile / pattern と metadata がずれないよう、詳細フォームでは直接編集しません。

`Sprites` は sprite asset tree、Frame Preview、Sprite Sheet、Animation Rows、Properties を持つ編集画面です。フレーム幅・高さと ROW ごとの有効 frame 数 / time を編集すると、PCE VN runtime が参照する `options.animations` と、エディタ再表示用の `options.spriteEditor` metadata に保存されます。Frame Preview と Sprite Sheet のプレビュー領域はマウスホイールで倍率を増減できます。Sprite Sheet のセルをクリックすると、その ROW / Frame が Frame Preview（画面上部）に反映され、Animation Rows の対応 ROW がハイライトされます。Sprite Sheet の各フレーム右上には、その frame の Time（既定 time）が表示されます。
...
- **Animation**: その立ち絵アセットの `options.animations`（Sprite エディタの ROW 定義）から再生するアニメーションを選びます。`default` 以外を選ぶと、ゲーム実機と同じく該当 ROW のフレームを `frameDelay` 間隔で巡回（ループ）します。
- **Move（0〜255 フレーム）**: 立ち絵を現在位置から新しい X/Y へスライド移動させるときのフレーム数です。**有効になる条件**は、(1) **同じスロットにすでに立ち絵が表示されている**、(2) 今回の `Sprite` コマンドも表示（visible）、(3) Move が 1 以上、の 3 つをすべて満たす場合だけです。条件を満たすと、毎フレーム **1 ピクセルずつ**目的座標へ近づけながら指定フレーム数だけ移動し、最後に目的座標へ確定します。そのため Move は所要時間であると同時に**移動速度（1 フレーム=1px）の上限**でもあります。移動距離（px）が Move フレーム数より大きいと、その分しか進まず最終フレームで残りを一気に目的地へスナップします。距離ぶんを滑らかに動かしたい場合は **Move ≧ 移動ピクセル数**にしてください。初めて表示するとき・非表示→表示の切り替え・`0` 指定では補間せず即座に目的座標へ配置されます。

右列のプレビューは、`BG` / `Sprite` などの画像系コマンドを選ぶと **256×224 のゲーム画面**として表示し、その時点までの背景・立ち絵の配置（背景は tile 座標、立ち絵はピクセル座標）を実際のレイアウトで確認できます。`Sprite` コマンドで指定した **Animation** はプレビュー上でもゲーム実機と同じフレーム切り出し・コマ送りで再生されるので、ROW 定義どおりに動くかをその場で確認できます。選択中コマンドが置いた要素は枠線でハイライトされます。
...
`Audio` コマンドの **Kind** には `CD-DA` / `ADPCM` に加えて **`PSG`** を選べます。`PSG` を選ぶと PSG 音源アセット（song / SFX）を再生でき、**基準 ch**（0〜5）で再生を始めるチャンネルを指定できます（song はループ、SFX は一度だけ）。`stop` で停止します。CD-DA 再生中にシーン変更や背景・スプライトの CD ロードが入る場合、ロード中は短く一時停止し、ロード後に同じトラックを再開します。ただし CD から直接ストリーミングする ADPCM はドライブを使い続けるため、CD-DA とは同時継続できません。
...
3. `Message` コマンドの **Mouth slot** に手順 2 と同じスロット番号を、**Mouth animation** に手順 1 のアニメーション名（例: `mouth`）を入力します。

スロットに立ち絵が表示されていない、または入力したアニメーション名が見つからない場合は無効（口パクなし）になります。口パクはメッセージ表示開始時に再生が始まり、メッセージ完了後も自動では止まりません。喋り終わりで口を閉じたいときは、その `Message` の後に同じスロットへ通常（idle）アニメーションを指定した `Sprite` コマンドを置いてください。
