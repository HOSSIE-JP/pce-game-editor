...
- **PSG（Sound タブの PSG）は `新規` で空のパターンを作るほか、`VGM取込` で既存の VGM / VGZ ファイルを登録**できます。取込ダイアログで ID・Name・BPM・Type（自動／SFX／Song）を指定すると、VGM 内の PC Engine（HuC6280）PSG レジスタ書き込みを BPM から決まる 16 分音符グリッド（最大 256 ステップ）へ量子化し、psg-song / psg-sfx アセットとして登録します。Type を「自動」にすると VGM のループ情報から song / SFX を判定します。波形・LFO・ノイズ・DDA は再現されず音程として近似されるため、短いジングルや効果音の取り込みに向いています。長い曲は先頭 256 ステップまでで切り詰められ、警告が表示されます。
