...
function ensurePsgPatternFiles(projectDir, doc, options = {}) {
  ...
  if (catalogMode) {
    // CD-ROM2 VN uses System Card main/sub packages generated by
    // pce-vn-manager. Remove stale legacy step-pattern payloads so they cannot
    // leak into the CD catalog or be mistaken for the current format.
...
function computeAssetMetaLayout(doc, options = {}) {
  ...
  const psg = options.includePsg === false
    ? []
    : assets.filter((a) => a.type === 'psg-song' || a.type === 'psg-sfx');
...
function ensureAssetMetaReservation(projectDir, doc) {
  const document = doc || readAssetDocument(projectDir);
  const useCdCatalog = assetMetaShouldUseCd(projectDir, document);
  // CD VN PSG is represented by System Card main/sub package refs in generated
  // vn.c. The legacy step-pattern descriptor has no slot in asset_meta.bin.
  const layout = computeAssetMetaLayout(document, { includePsg: !useCdCatalog });
...
function collectCdDataFilesForDocument(projectDir, doc) {
  ...
  (doc.assets || []).forEach((asset) => {
    ...
    if (asset.type === 'image' || asset.type === 'sprite') {
      ...
      if (!catalogMode && psgPatternBytes(asset).length && psgAssetStreamsFromCd(asset, true, { catalogMode })) files.push(psgPatternFile(asset));
    }
  });
...
function generateAssetSources(projectDir, options = {}) {
  ...
  const psgGenerated = generatePsgMetadata(projectDir, assetMetaOnCd ? [] : sourceDoc.assets, {
    targetsCd,
    cdLayout,
    catalogMode: false,
  });
  ...
  const linesH = [
    ...
    ...(assetMetaOnCd ? [] : ['extern const pce_editor_meta_region_t pce_editor_psg_meta;']),
