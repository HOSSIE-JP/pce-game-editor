...
- `game-editor-common.js` — 共有アプリ基盤を本体に取り込み、独自にメンテナンスするモジュール (旧 `../game-editor-common` 外部パッケージは廃止)。
...
- 編集前に baseline として `npm test` を実行し結果を記録する。
...
`npm test` が環境要因で実行できない場合は、その理由を記録し、人間に報告してから作業可否の判断を仰ぐこと。
