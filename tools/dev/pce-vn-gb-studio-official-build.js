...
async function runOfficialBuild({ executablePath, projectPath, outputDir, gbStudioVersion = '', engineVersion = '' } = {}) {
  ...
  try {
    ...
    if (!main) { await evaluateOnce(firstPage, `Promise.resolve(API.project.openProject(${JSON.stringify(project)})).then(() => true)`); main = await waitTargets(port, (target) => target.type === 'page' && target.url.includes('main_window'), 240); }
    ...
    return { status: 'pass', gbStudioVersion: String(gbStudioVersion || ''), engineVersion: String(engineVersion || ''), title: loaded.title, projectName: loaded.projectName, startedAt: new Date(startedAt).toISOString(), build, webBuild, rom: { path: romPath, bytes: rom.length, sha256: romHash, cgbFlag: '0x80', title: rom.subarray(0x134, 0x143).toString('ascii').replace(/\0+$/, '') }, web: { indexPath: webIndex, files: webFiles.length, sha256: sha256(webIndex), romPath: webRom, romSha256: webRomHash, matchesStandaloneRom: true }, process: { pid: child.pid, profile, stdout, stderr } };
