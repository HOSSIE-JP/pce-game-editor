...
function main() {
  ...
  const projectDir = path.resolve(options.projectDir); const settings = { cddaMappings: options.cddaMappings, audioSubstitutions: options.audioSubstitutions }; for (const key of ['font', 'portraitRenderMode', 'visualOmissionsConfirmed', 'warningsAcknowledged']) if (options[key] !== undefined) settings[key] = options[key]; const inspection = inspectGbStudioExport({ projectDir, settings, gbStudio: options.gbStudio });
