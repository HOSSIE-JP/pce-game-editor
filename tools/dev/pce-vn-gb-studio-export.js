...
function printHuman(inspection) { const summary = inspection.summary || {}; process.stdout.write(`Preflight: ${inspection.ok ? 'OK' : 'BLOCKED'} / scene ${summary.sourceScenes || 0} -> ${summary.outputScenes || 0} / BG ${summary.backgroundVariants || 0} / font ${summary.fontPages || 0} page / BGM ${summary.musicTracks || 0}\n`); [...inspection.errors, ...inspection.warnings].forEach((entry) => process.stdout.write(`${entry.severity.toUpperCase()} ${entry.code} ${entry.location}: ${entry.message}\n`)); }
...
function main() {
  ...
  const projectDir = path.resolve(options.projectDir); const inspection = inspectGbStudioExport({ projectDir, settings: { font: options.font, visualOmissionsConfirmed: options.visualOmissionsConfirmed, warningsAcknowledged: options.warningsAcknowledged, cddaMappings: options.cddaMappings, audioSubstitutions: options.audioSubstitutions }, gbStudio: options.gbStudio });
