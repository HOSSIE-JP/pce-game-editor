...
const { bakedSpritePreviewFrames, normalizePortraitRenderMode, normalizeSpriteSetting, transformSpriteAsset } = require('./pce-vn-gb-studio-visual');

const EXPORTER_FORMAT = 'pce-vn-gb-studio-export';
const EXPORTER_VERSION = 1;
const EXPORTER_RELEASE_VERSION = '1.4.0';
...
function normalizeSidecar(value = {}) {
  const raw = value && typeof value === 'object' ? value : {}; return { format: EXPORTER_FORMAT, version: EXPORTER_VERSION, font: String(raw.font || DEFAULT_FONT), portraitRenderMode: normalizePortraitRenderMode(raw.portraitRenderMode), visualOmissionsConfirmed: Boolean(raw.visualOmissionsConfirmed), visualOmissionsConfirmationHash: /^[0-9a-f]{64}$/i.test(String(raw.visualOmissionsConfirmationHash || '')) ? String(raw.visualOmissionsConfirmationHash).toLowerCase() : '', warningsAcknowledged: Boolean(raw.warningsAcknowledged), warningsAcknowledgementHash: /^[0-9a-f]{64}$/i.test(String(raw.warningsAcknowledgementHash || '')) ? String(raw.warningsAcknowledgementHash).toLowerCase() : '', cddaMappings: raw.cddaMappings && typeof raw.cddaMappings === 'object' ? raw.cddaMappings : {}, audioSubstitutions: raw.audioSubstitutions && typeof raw.audioSubstitutions === 'object' ? raw.audioSubstitutions : {}, backgrounds: normalizeSettingMap(raw.backgrounds, normalizeBackgroundSetting), sprites: normalizeSettingMap(raw.sprites || raw.sprite, normalizeSpriteSetting), music: normalizeSettingMap(raw.music, normalizeMusicTrackSettings) };
}
...
function mergedSettings(projectDir, explicit = {}) {
  const sidecar = readSidecar(projectDir); const backgroundIds = new Set([...Object.keys(sidecar.backgrounds || {}), ...Object.keys(explicit.backgrounds || {})]); const spriteIds = new Set([...Object.keys(sidecar.sprites || {}), ...Object.keys(explicit.sprites || explicit.sprite || {})]); const musicIds = new Set([...Object.keys(sidecar.music || {}), ...Object.keys(explicit.music || {})]);
  const backgrounds = Object.fromEntries([...backgroundIds].map((id) => [id, { ...(sidecar.backgrounds?.[id] || {}), ...(explicit.backgrounds?.[id] || {}) }]));
  const sprites = Object.fromEntries([...spriteIds].map((id) => [id, { ...(sidecar.sprites?.[id] || {}), ...((explicit.sprites || explicit.sprite)?.[id] || {}) }]));
  ...
  return normalizeSidecar({ ...sidecar, ...explicit, cddaMappings: { ...sidecar.cddaMappings, ...(explicit.cddaMappings || {}) }, audioSubstitutions: { ...sidecar.audioSubstitutions, ...(explicit.audioSubstitutions || {}) }, backgrounds, sprites, music });
}

function confirmationSettings(settings) {
  const output = { ...settings, visualOmissionsConfirmed: false, visualOmissionsConfirmationHash: '', warningsAcknowledged: false, warningsAcknowledgementHash: '' }; return output;
}
...
function previewVnGbStudioBackground({ projectDir, assets, assetId, fullScreen = false, settings = {}, generation = 0 } = {}) {
  ...
  const backgroundSetting = settings.backgrounds?.[id] || settings; const normalized = normalizeBackgroundSetting(backgroundSetting); const model = { projectDir: context.root, assetsById: context.assetsById, settings: { backgrounds: { [id]: normalized } } }; const variant = { key: `${id}:${fullScreen ? 'fullscreen' : 'dialogue'}`, assetId: id, fullScreen: Boolean(fullScreen) }; const transformed = transformBackgroundVariant(model, variant, { includeSourceImage: true });
...
function previewVnGbStudioSprite({ projectDir, assets, assetId, settings = {}, renderMode = 'baked', mode = 'gbc', usageSceneId = '', animationId = '', generation = 0 } = {}) {
  const context = previewProject(projectDir, assets); const id = validatePreviewAssetId(assetId); const asset = context.assetsById.get(id); if (!asset || asset.type !== 'sprite') throw previewError('GBVN_PREVIEW_ASSET_INVALID', `sprite assetを解決できません: ${id}`);
  const resolved = resolveAssetSource(context.root, asset).absPath; if (!isPathInside(context.root, resolved)) throw previewError('GBVN_PREVIEW_PATH_OUTSIDE_PROJECT', `project外のsprite素材はpreviewできません: ${id}`); if (!fs.existsSync(resolved) || fs.statSync(resolved).size > MAX_PREVIEW_SOURCE_BYTES) throw previewError('GBVN_PREVIEW_DATA_TOO_LARGE', `${id}: source PNGがpreview上限を超えるか存在しません`);
  const normalizedRenderMode = normalizePortraitRenderMode(renderMode); const combinedSettings = mergedSettings(context.root, settings); combinedSettings.portraitRenderMode = normalizedRenderMode; const spriteSettings = combinedSettings.sprites?.[id] || settings.sprites?.[id] || settings.sprite?.[id] || settings; const transformed = transformSpriteAsset({ projectDir: context.root, asset, settings: spriteSettings, renderMode: normalizedRenderMode, includeSourceImage: true }); const selectedMode = mode === 'dmg' ? 'dmg' : 'gbc'; const animation = transformed.animations.find((entry) => entry.id === animationId) || transformed.animations[0];
  let frameEntries = animation.frames.map((entry) => transformed.modes[selectedMode].frames[transformed.frameIndex[entry.key]]); let hashes = transformed.outputHashes; let frameHashes = { gbc: [], dmg: [] }; let bakedContext = null;
  if (normalizedRenderMode === 'baked') {
    const scenePath = path.join(context.root, 'assets', 'pce-vn-scenes.json'); const sceneDoc = normalizeSceneDocument(readJson(scenePath, 'assets/pce-vn-scenes.json'), context.assetDoc); const scene = sceneDoc.scenes.find((entry) => entry.id === String(usageSceneId || '')) || sceneDoc.scenes.find((entry) => entry.commands.some((command) => command.type === 'sprite' && command.assetId === id && command.visible !== false));
    const spriteCommandIndex = scene ? scene.commands.findIndex((command) => command.type === 'sprite' && command.assetId === id && command.visible !== false && (!animationId || command.animationId === animation.id)) : -1; const fallbackSpriteIndex = scene ? scene.commands.findIndex((command) => command.type === 'sprite' && command.assetId === id && command.visible !== false) : -1; const usageIndex = spriteCommandIndex >= 0 ? spriteCommandIndex : fallbackSpriteIndex; const usage = usageIndex >= 0 ? scene.commands[usageIndex] : { slot: 0, x: 0, y: 0, flipX: false, flipY: false };
    const backgroundCommands = scene ? scene.commands.slice(0, usageIndex >= 0 ? usageIndex + 1 : undefined).filter((command) => command.type === 'background') : []; const background = backgroundCommands.at(-1) || scene?.commands.find((command) => command.type === 'background') || null; const model = { projectDir: context.root, assetsById: context.assetsById, settings: combinedSettings, visual: { spriteTransforms: new Map([[id, transformed]]) }, font: { assignments: {}, pages: [] }, textUnits: [] }; const variant = transformBackgroundVariant(model, { key: `preview:${scene?.id || 'unplaced'}:${id}`, assetId: background?.assetId || '', fullScreen: Boolean(scene?.fullScreenBg), visualState: null }, { includeSourceImage: true }); const previewOptions = { animationId: animation.id, slot: usage.slot, x: usage.x, y: usage.y, flipX: usage.flipX, flipY: usage.flipY };
    const allModes = Object.fromEntries(['gbc', 'dmg'].map((previewMode) => [previewMode, bakedSpritePreviewFrames(model, variant, transformed, { ...previewOptions, mode: previewMode })])); frameEntries = allModes[selectedMode].map((entry) => ({ image: entry.converted })); frameHashes = Object.fromEntries(Object.entries(allModes).map(([previewMode, entries]) => [previewMode, entries.map((entry) => entry.outputHash)])); hashes = { gbc: frameHashes.gbc[0] || '', dmg: frameHashes.dmg[0] || '' }; bakedContext = { sceneId: scene?.id || '', commandIndex: usageIndex, backgroundAssetId: background?.assetId || '', fullScreen: Boolean(scene?.fullScreenBg), position: { x: Number(usage.x) || 0, y: Number(usage.y) || 0 }, flipX: Boolean(usage.flipX), flipY: Boolean(usage.flipY), frameHashes };
  }
  const frame = frameEntries[0]; return { ok: true, generation: Number(generation) || 0, renderMode: normalizedRenderMode, mode: selectedMode, usageSceneId: String(usageSceneId || ''), animationId: animation.id, animation: { id: animation.id, loop: animation.loop, frameDelays: animation.delays, generatedTiming: transformed.timing.animations.find((entry) => entry.id === animation.id) }, asset: { id, name: asset.name || id }, settings: { ...transformed.settings, sourceHash: transformed.sourceHash }, stale: transformed.stale, crop: transformed.crop, audit: { ...transformed.audit, ...(bakedContext ? { bakedPreview: bakedContext } : {}) }, hashes, frameHashes, images: { source: previewDataUrl(transformed.sourceImage, `${id}/source`), frame: previewDataUrl(frame.image, `${id}/${selectedMode}`), frames: frameEntries.map((entry, index) => previewDataUrl(entry.image, `${id}/${selectedMode}/frame-${index}`)), sheet: previewDataUrl(transformed.modes[selectedMode].sheet, `${id}/${selectedMode}/sheet`) } };
}
...
function validateCommands(sceneDoc, assetDoc, settings, projectDir) {
  ...
  for (const scene of sceneDoc.scenes || []) {
    ...
    for (let index = 0; index < (scene.commands || []).length; index += 1) {
      ...
      if (command.type === 'sprite' && command.visible !== false && (!command.assetId || assets.get(command.assetId)?.type !== 'sprite')) add('error', 'GBVN_UNRESOLVED_ASSET', `sprite assetを解決できません: ${command.assetId || '(空)'}`, location);
      ...
      if (command.type === 'choice') {
        ...
        for (const choice of command.choices || []) if (choice.targetSceneId && !sceneIds.has(choice.targetSceneId)) add('error', 'GBVN_UNRESOLVED_SCENE', `choice先sceneを解決できません: ${choice.targetSceneId}`, location);
      }
      ...
      if (command.type === 'effect' && !['fadeIn', 'fadeOut', 'flash', 'blank', 'shake'].includes(command.effect)) add('error', 'GBVN_UNSUPPORTED_VISUAL_COMMAND', `未対応effectです: ${command.effect}`, location);
      if (command.type === 'cache') omissions.push({ sceneId: scene.id, commandIndex: sourceIndex, type: 'cache', reason: 'GB Studioでは事前cache不要', requiresConfirmation: false });
...
function inspectGbStudioExport({ projectDir, doc, assets, settings: explicitSettings = {}, gbStudio } = {}) {
  ...
  try {
    ...
    if (model) {
      ...
      const visualIssues = new Set();
      for (const segment of model.graph.segments) for (const plan of segment.visualPlans || []) {
        const location = plan.sourceKey ? `${plan.sourceKey.split(':')[0]}.commands[${plan.sourceKey.split(':').at(-1)}]` : `graph:${segment.key}`;
        if (plan.missingSlot && !visualIssues.has(`${plan.sourceKey}:missing`)) { visualIssues.add(`${plan.sourceKey}:missing`); diagnostics.push(diagnostic('error', 'GBVN_SPRITE_SLOT_UNDEFINED', 'spritemove対象slotに論理sprite状態がありません', location, plan)); }
        const command = segment.commands[plan.commandIndex]; const beforeSprite = plan.beforeState?.sprites?.[Number(command?.slot) || 0];
        if (command?.type === 'spritemove' && command.animationId && beforeSprite?.assetId !== command.animationAssetId && !visualIssues.has(`${plan.sourceKey}:asset`)) { visualIssues.add(`${plan.sourceKey}:asset`); diagnostics.push(diagnostic('error', 'GBVN_SPRITE_ANIMATION_ASSET_MISMATCH', `spritemove animation asset ${command.animationAssetId} がslotの ${beforeSprite?.assetId || '(none)'} と一致しません`, location, plan)); }
      }
      const structuralErrorCodes = new Set(['GBVN_UNKNOWN_COMMAND', 'GBVN_UNCONSUMED_COMMAND', 'GBVN_UNRESOLVED_ASSET', 'GBVN_UNRESOLVED_SCENE', 'GBVN_UNSUPPORTED_CONTROL_COMMAND', 'GBVN_CHOICE_OPTION_COUNT', 'GBVN_CHOICE_MENU_OVERFLOW', 'GBVN_SPRITE_SLOT_UNDEFINED', 'GBVN_SPRITE_ANIMATION_ASSET_MISMATCH']);
      const skipBackgroundTransform = diagnostics.some((entry) => entry.severity === 'error' && structuralErrorCodes.has(entry.code));
      if (!skipBackgroundTransform) try { backgrounds = transformBackgrounds(model); backgrounds.audits.forEach((audit) => { if (audit.gbc.palettes > 7 || audit.gbc.maxColorsPerTile > 4) diagnostics.push(diagnostic('error', 'GBVN_GBC_PALETTE_OVERFLOW', `${audit.key}: GBC palette制約を超えています`, `background:${audit.key}`, audit.gbc)); if (audit.dmg.uniqueTiles > 192) diagnostics.push(diagnostic('error', 'GBVN_DMG_TILE_OVERFLOW', `${audit.key}: DMG固有tileが192を超えています`, `background:${audit.key}`, audit.dmg)); if (audit.dmg.meaningfulShades < 4) diagnostics.push(diagnostic('warning', 'GBVN_DMG_SHADE_UNDERUSE', `${audit.key}: 原画の階調が少ないためDMG 4階調の一部を有意量使用していません`, `background:${audit.key}`, audit.dmg)); }); backgrounds.visual.sprites.filter((audit) => audit.stale).forEach((audit) => diagnostics.push(diagnostic('warning', 'GBVN_SPRITE_SETTING_STALE', `${audit.assetId}: source hashが保存済み立ち絵設定と一致しません。設定を保持したまま再previewしてください`, `sprite:${audit.assetId}`, audit))); backgrounds.visual.omissions.forEach((audit) => { const location = audit.sourceKey || `visual:${audit.id || audit.visualStateId || 'state'}`; diagnostics.push(diagnostic('warning', 'GBVN_VISUAL_OMISSION_REQUIRES_CONFIRMATION', `${location}: ${audit.omittedAttributes.map((entry) => entry.attribute).join(', ')}を属性単位で省略します`, location, audit)); }); if (backgrounds.visual.approximations.length) diagnostics.push(diagnostic('warning', 'GBVN_VISUAL_APPROXIMATION_REQUIRES_ACKNOWLEDGEMENT', `visual timing・色・tile更新の近似 ${backgrounds.visual.approximations.length}件をvisual-audit.jsonへ記録します`, 'visual-audit', { count: backgrounds.visual.approximations.length, auditHash: backgrounds.visual.auditHash || '' })); } catch (error) { diagnostics.push(diagnostic('error', error.code || 'GBVN_UNRESOLVED_ASSET', String(error?.message || error), 'backgrounds', error.pair || {})); }
    ...
    commandValidation.omissions.filter((omission) => omission.requiresConfirmation !== false).forEach((omission) => diagnostics.push(diagnostic('warning', 'GBVN_VISUAL_OMISSION_REQUIRES_CONFIRMATION', `${omission.type}を変換対象から省略します: ${omission.reason}`, `${omission.sceneId}.commands[${omission.commandIndex}]`, omission)));
    ...
    const spriteCatalog = backgrounds ? backgrounds.visual.sprites.map((audit) => { const asset = assetDoc.assets.find((entry) => entry.id === audit.assetId); const usageSceneIds = [...new Set(sceneDoc.scenes.filter((scene) => scene.commands.some((command) => (command.type === 'sprite' && command.assetId === audit.assetId) || (command.type === 'spritemove' && command.animationAssetId === audit.assetId))).map((scene) => scene.id))]; return { id: audit.assetId, name: asset?.name || audit.assetId, usageSceneIds, animations: (backgrounds.spriteTransforms.get(audit.assetId)?.animations || []).map((animation) => ({ id: animation.id, name: animation.name, frameCount: animation.frames.length, loop: animation.loop })), settings: { ...(settings.sprites?.[audit.assetId] || normalizeSpriteSetting()), sourceHash: audit.sourceHash }, stale: audit.stale, metrics: { gbc: audit.gbc, dmg: audit.dmg } }; }) : [];
    for (const audit of backgrounds?.visual?.sprites || []) settings.sprites[audit.assetId] = { ...(settings.sprites[audit.assetId] || normalizeSpriteSetting()), sourceHash: audit.sourceHash };
    const errors = diagnostics.filter((entry) => entry.severity === 'error'); const warnings = diagnostics.filter((entry) => entry.severity === 'warning'); const omissions = [...commandValidation.omissions, ...(backgrounds?.visual?.omissions || [])];
    const confirmationSnapshot = sourceSnapshotRaw(root, sceneDoc, assetDoc, confirmationSettings(settings), rawDoc); const confirmationInputHash = sha256(stableJson({ sourceSignature: confirmationSnapshot.signature, gbStudio: { version: installation.version, engineVersion: installation.engineVersion } })); const visualOmissionsHash = sha256(stableJson({ input: confirmationInputHash, omissions })); const acknowledgementWarnings = warnings.filter((entry) => entry.code !== 'GBVN_VISUAL_OMISSION_REQUIRES_CONFIRMATION'); const warningsAcknowledgementHash = sha256(stableJson({ input: confirmationInputHash, warnings: acknowledgementWarnings }));
    const explicitVisualConfirmation = Object.prototype.hasOwnProperty.call(explicitSettings || {}, 'visualOmissionsConfirmed'); const explicitWarningAcknowledgement = Object.prototype.hasOwnProperty.call(explicitSettings || {}, 'warningsAcknowledged'); const staleVisualConfirmation = Boolean(settings.visualOmissionsConfirmed && !explicitVisualConfirmation && settings.visualOmissionsConfirmationHash !== visualOmissionsHash); const staleWarningAcknowledgement = Boolean(settings.warningsAcknowledged && !explicitWarningAcknowledgement && settings.warningsAcknowledgementHash !== warningsAcknowledgementHash);
    if (staleVisualConfirmation) settings.visualOmissionsConfirmed = false; if (staleWarningAcknowledgement) settings.warningsAcknowledged = false; settings.visualOmissionsConfirmationHash = settings.visualOmissionsConfirmed ? visualOmissionsHash : ''; settings.warningsAcknowledgementHash = settings.warningsAcknowledged ? warningsAcknowledgementHash : ''; const snapshot = sourceSnapshotRaw(root, sceneDoc, assetDoc, settings, rawDoc);
    ...
    const result = { ok: errors.length === 0, format: EXPORTER_FORMAT, version: EXPORTER_VERSION, exporterReleaseVersion: EXPORTER_RELEASE_VERSION, projectDir: root, project: { title: project.title || project.name || '', romName: project.romName || '' }, settings, confirmations: { inputHash: confirmationInputHash, visualOmissionsHash, warningsAcknowledgementHash, staleVisualConfirmation, staleWarningAcknowledgement }, gbStudio: installation, sourceSignature: snapshot.signature, sourceFiles: snapshot.files, errors, warnings, omissions, requirements: { cdda: [...referencedCdda].map((id) => ({ id, name: assetDoc.assets.find((asset) => asset.id === id)?.name || id, mapping: settings.cddaMappings[id] || '' })), audioSubstitutions: [...referencedSubstitutions].map((id) => ({ id, name: assetDoc.assets.find((asset) => asset.id === id)?.name || id, type: assetDoc.assets.find((asset) => asset.id === id)?.type || '', mapping: settings.audioSubstitutions[id] || '' })), automaticVoiceSubstitutions, psgSongs, backgrounds: backgroundCatalog, sprites: spriteCatalog, reachableActorPairs: backgrounds?.visual?.actorPairs || [], visualTimelines: backgrounds?.visual?.timelines || [], generatedPlugins, customEventReferences }, summary: { sourceScenes: sceneDoc.scenes.length, sourceCommands: inventory.commands.length, outputScenes: model ? model.graph.segments.length * 2 + 1 : 0, backgroundVariants: model ? model.backgroundVariants.length : 0, spriteAssets: spriteCatalog.length, actorPairs: backgrounds?.visual?.actorPairs?.length || 0, visualTimelines: backgrounds?.visual?.timelines?.length || 0, portraitRenderMode: settings.portraitRenderMode, fontGlyphs: model ? model.font.glyphCount : 0, fontPages: model ? model.font.pages.length : 0, musicTracks: model ? model.music.length + (model.externalMusic?.length || 0) : 0, variables: model ? model.variables.names.length : 0, automaticVoiceSubstitutions: automaticVoiceSubstitutions.length, choices: choiceCounts, reachableBlocks: model ? model.graph.reachable.length : 0, unreachableBlocks: model ? model.graph.unreachable.length : 0, generatedPlugins: generatedPlugins.length, customEventReferences: customEventReferences.length, commandConsumption }, audits: { backgrounds: backgrounds?.audits || [], music: music?.audits || [], visual: backgrounds?.visual || {}, controlFlow: model ? { startKey: model.graph.startKey, reachable: model.graph.reachable, unreachable: model.graph.unreachable, joins: model.graph.joins, loops: model.graph.loops, segments: model.graph.segments.map((segment) => ({ key: segment.key, originBlockKey: segment.originBlockKey, reachable: segment.reachable, entryBackgroundKey: segment.entryBackgroundKey, effectiveBackgroundKey: segment.effectiveBackgroundKey, backgroundSource: segment.backgroundSource, visualStateId: segment.visualStateId })) } : {} } };
...
function staticValidateFiles(files) {
  const errors = []; const resources = { scenes: new Map(), actors: new Map(), scripts: new Map(), backgrounds: new Set(), sprites: new Map(), spriteStateNames: new Set(), tilesets: new Set(), palettes: new Set(), fonts: new Map(), music: new Set(), variables: new Set() }; const customEvents = new Set(); const eventIds = new Set(); let settings = null; let descriptor = 0;
  for (const [relative, data] of files) {
    ...
    try { const value = JSON.parse(Buffer.from(data).toString('utf-8')); if (value._resourceType === 'settings') settings = value; if (value._resourceType === 'scene') resources.scenes.set(value.id, value); if (value._resourceType === 'actor') resources.actors.set(value.id, value); if (value._resourceType === 'script') resources.scripts.set(value.id, value); if (value._resourceType === 'background') resources.backgrounds.add(value.id); if (value._resourceType === 'sprite') { resources.sprites.set(value.id, value); for (const state of value.states || []) resources.spriteStateNames.add(String(state.name || '')); } if (value._resourceType === 'tileset') resources.tilesets.add(value.id); if (value._resourceType === 'palette') resources.palettes.add(value.id); if (value._resourceType === 'font') resources.fonts.set(value.id, value); if (value._resourceType === 'music') resources.music.add(value.id); if (value._resourceType === 'variables') for (const variable of value.variables || []) resources.variables.add(String(variable.id)); if (value.filename && (path.isAbsolute(value.filename) || String(value.filename).includes('..'))) errors.push(`resource filenameが不正です: ${relative}`); if (value.filename) { const prefix = { background: 'assets/backgrounds', sprite: 'assets/sprites', tileset: 'assets/tilesets', font: 'assets/fonts', music: 'assets/music' }[value._resourceType]; if (prefix && !files.has(safeRelative(path.posix.join(prefix, String(value.filename).replace(/\\/g, '/'))))) errors.push(`${value.id}: resource画像/dataがありません ${value.filename}`); } } catch (error) { errors.push(`JSON resourceを読めません: ${relative}: ${error.message}`); }
  }
  ...
  const scanEvents = (events, location) => {
    ...
    for (const item of events || []) {
      ...
      if (item.command === 'EVENT_CALL_CUSTOM_EVENT' && !resources.scripts.has(item.args?.customEventId)) errors.push(`${location}: custom script参照を解決できません ${item.args?.customEventId}`);
      ...
      if (/^EVENT_ACTOR_/.test(String(item.command || '')) && item.args?.actorId && !['player', '$self$'].includes(item.args.actorId) && !resources.actors.has(item.args.actorId)) errors.push(`${location}: actor参照を解決できません ${item.args.actorId}`);
      if (item.command === 'EVENT_ACTOR_SET_SPRITE' && !resources.sprites.has(item.args?.spriteSheetId)) errors.push(`${location}: sprite参照を解決できません ${item.args?.spriteSheetId}`); if (item.command === 'EVENT_ACTOR_SET_STATE' && item.args?.spriteStateId && !resources.spriteStateNames.has(String(item.args.spriteStateId))) errors.push(`${location}: sprite state名を解決できません ${item.args.spriteStateId}`); if (item.command === 'EVENT_REPLACE_TILE_XY' && !resources.tilesets.has(item.args?.tilesetId)) errors.push(`${location}: tileset参照を解決できません ${item.args?.tilesetId}`);
      if (item.command === 'EVENT_PALETTE_SET_SPRITE') for (let index = 0; index < 8; index += 1) { const palette = item.args?.[`palette${index}`]; if (palette && !['keep', 'restore'].includes(palette) && !resources.palettes.has(palette)) errors.push(`${location}: sprite palette参照を解決できません ${palette}`); }
      Object.entries(item.children || {}).forEach(([key, children]) => scanEvents(children, `${location}/${item.id}/${key}`));
    }
  };
  for (const scene of resources.scenes.values()) { if (!resources.backgrounds.has(scene.backgroundId)) errors.push(`${scene.id}: background参照を解決できません ${scene.backgroundId}`); for (const palette of scene.spritePaletteIds || []) if (palette && !resources.palettes.has(palette)) errors.push(`${scene.id}: scene sprite palette参照を解決できません ${palette}`); scanEvents(scene.script, scene.id); }
  for (const actor of resources.actors.values()) { if (!resources.sprites.has(actor.spriteSheetId)) errors.push(`${actor.id}: actor sprite参照を解決できません ${actor.spriteSheetId}`); if (actor.paletteId && !resources.palettes.has(actor.paletteId)) errors.push(`${actor.id}: actor palette参照を解決できません ${actor.paletteId}`); for (const field of ['script', 'startScript', 'updateScript', 'hit1Script', 'hit2Script', 'hit3Script']) scanEvents(actor[field], `${actor.id}/${field}`); }
  for (const script of resources.scripts.values()) scanEvents(script.script, `${script.id}/script`);
  ...
  const visualAuditPath = 'build/qa/visual-audit.json'; if (!files.has(visualAuditPath)) errors.push('visual-audit.jsonがありません'); else try { const audit = JSON.parse(Buffer.from(files.get(visualAuditPath)).toString('utf-8')); if (audit.status !== 'pass' || audit.failures?.length) errors.push('visual監査がpassではありません'); for (const command of audit.commands || []) { for (const id of Object.values(command.actorIds || {})) if (!resources.actors.has(id)) errors.push(`${command.sourceKey}: visual audit actorを解決できません ${id}`); for (const id of Object.values(command.tilesetIds || {})) if (!resources.tilesets.has(id)) errors.push(`${command.sourceKey}: visual audit tilesetを解決できません ${id}`); for (const ids of Object.values(command.customScriptIds || {})) for (const id of ids || []) if (!resources.scripts.has(id)) errors.push(`${command.sourceKey}: visual audit custom scriptを解決できません ${id}`); for (const mode of ['gbc', 'dmg']) for (const id of command.generatedEvents?.[mode]?.eventIds || []) if (!eventIds.has(id)) errors.push(`${command.sourceKey}: visual audit eventを解決できません ${id}`); } } catch (error) { errors.push(`visual-audit.jsonを読めません: ${error.message}`); }
  return { ok: errors.length === 0, errors, counts: { scenes: resources.scenes.size, actors: resources.actors.size, scripts: resources.scripts.size, backgrounds: resources.backgrounds.size, sprites: resources.sprites.size, tilesets: resources.tilesets.size, palettes: resources.palettes.size, fonts: resources.fonts.size, music: resources.music.size, variables: resources.variables.size, customEvents: customEvents.size } };
}
function portableSidecar(projectDir, settings, inspection) {
  ...
  for (const audit of inspection?.audits?.visual?.sprites || []) sidecar.sprites[audit.assetId] = { ...(sidecar.sprites[audit.assetId] || normalizeSpriteSetting()), sourceHash: audit.sourceHash };
  const portable = { ...sidecar, sprite: sidecar.sprites }; delete portable.sprites;
  return { sidecar: portable, copies };
}
...
function commitNewOutput(stageDir, outputDir, files) {
  try { fs.renameSync(stageDir, outputDir); return 'rename'; } catch (error) {
    if (!['EPERM', 'EACCES'].includes(error?.code) || fs.existsSync(outputDir)) throw error;
    const parent = path.dirname(outputDir); if (!isPathInside(parent, outputDir) || !isPathInside(parent, stageDir) || !path.basename(stageDir).startsWith('.pce-vn-gb-stage-')) throw new Error('copy fallback安全検査に失敗しました');
    try {
      fs.mkdirSync(outputDir, { recursive: false }); fs.cpSync(stageDir, outputDir, { recursive: true, force: false, errorOnExist: true });
      for (const [relative, data] of files) { const destination = path.resolve(outputDir, safeRelative(relative)); if (!isPathInside(outputDir, destination) || !fs.existsSync(destination) || fs.statSync(destination).size !== Buffer.byteLength(data)) throw new Error(`copy fallback検証に失敗しました: ${relative}`); }
      fs.rmSync(stageDir, { recursive: true, force: true }); return 'copy-fallback';
    } catch (copyError) { if (fs.existsSync(outputDir) && isPathInside(parent, outputDir)) fs.rmSync(outputDir, { recursive: true, force: true }); throw copyError; }
  }
}
...
function commitFileMap(outputDir, stageDir, files, oldManifest) {
  ...
  if (!existed || existingFiles.length === 0) { if (existed) fs.rmdirSync(outputDir); const commitMode = commitNewOutput(stageDir, outputDir, files); return { backupPath, commitMode }; }
  ...
  const stageParent = path.dirname(stageDir); if (!isPathInside(stageParent, stageDir) || !path.basename(stageDir).startsWith('.pce-vn-gb-stage-')) throw new Error('stage cleanup安全検査に失敗しました'); fs.rmSync(stageDir, { recursive: true, force: true }); return { backupPath, commitMode: 'owned-update' };
}
...
function generateGbStudioProject({ inspection, outputDir, mode = 'generate' } = {}) {
  ...
  const visualOmissions = inspection.audits?.visual?.omissions || []; if (visualOmissions.length && !inspection.settings?.visualOmissionsConfirmed) { const error = new Error(`visual属性省略 ${visualOmissions.length}件の明示確認が必要です`); error.code = 'GBVN_VISUAL_OMISSION_REQUIRES_CONFIRMATION'; error.diagnostics = visualOmissions; throw error; }
  const acknowledgementWarnings = (inspection.warnings || []).filter((entry) => entry.code !== 'GBVN_VISUAL_OMISSION_REQUIRES_CONFIRMATION'); if (acknowledgementWarnings.length && !inspection.settings?.warningsAcknowledged) { const error = new Error(`warning ${acknowledgementWarnings.length}件の明示確認が必要です`); error.code = 'GBVN_OFFICIAL_BUILD_WARNING'; error.diagnostics = acknowledgementWarnings; throw error; }
  ...
  try {
    ...
    return { ok: validation.ok, outputDir: target, descriptorPath: path.join(target, built.descriptor), manifestPath: path.join(target, MANIFEST_FILE), sidecarPath: path.join(inspection.projectDir, SIDECAR_FILE), backupPath: commit.backupPath, commitMode: commit.commitMode, stats: built.stats, officialBuild, validation, runtime: { ran: false, code: 'GBVN_RUNTIME_NOT_RUN', message: 'GB Studio内蔵emulatorの実入力smokeは未実行です' } };
...
function validateGbStudioProject({ outputDir, inspection, requireBuild = false } = {}) {
  ...
  const files = new Map(); for (const absolute of walkFiles(root)) { const relative = normalizeRelativePath(path.relative(root, absolute)); if (relative.endsWith('.gbsres') || relative.endsWith('.gbsproj') || relative.startsWith('plugins/') || relative.startsWith('assets/') || relative === 'build/qa/control-flow-audit.json' || relative === 'build/qa/visual-audit.json') files.set(relative, fs.readFileSync(absolute)); }
