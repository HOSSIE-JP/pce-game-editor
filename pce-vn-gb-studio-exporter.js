...
function staticValidateFiles(files) {
  const errors = []; const resources = { scenes: new Map(), backgrounds: new Set(), fonts: new Map(), music: new Set() }; let settings = null; let descriptor = 0;
  for (const [relative, data] of files) {
    if (relative.endsWith('.gbsproj')) descriptor += 1; if (!relative.endsWith('.gbsres')) continue;
    try { const value = JSON.parse(Buffer.from(data).toString('utf-8')); if (value._resourceType === 'settings') settings = value; if (value._resourceType === 'scene') resources.scenes.set(value.id, value); if (value._resourceType === 'background') resources.backgrounds.add(value.id); if (value._resourceType === 'font') resources.fonts.set(value.id, value); if (value._resourceType === 'music') resources.music.add(value.id); if (value.filename && (path.isAbsolute(value.filename) || String(value.filename).includes('..'))) errors.push(`resource filenameが不正です: ${relative}`); } catch (error) { errors.push(`JSON resourceを読めません: ${relative}: ${error.message}`); }
  }
...
function manifestFor(files, inspection, stats) {
  ...
  return { format: EXPORTER_FORMAT, version: EXPORTER_VERSION, exporter: { id: 'pce-vn-gb-studio-exporter', version: '1.1.3' }, generatedAt: new Date().toISOString(), sourceProject: { identity: sourceProjectIdentity(inspection.projectDir), title: inspection.project?.title || '', romName: inspection.project?.romName || '' }, sourceSignature: inspection.sourceSignature, gbStudio: { version: inspection.gbStudio.version, engineVersion: inspection.gbStudio.engineVersion }, conversion: { font, cddaMappings: stableValue(inspection.settings.cddaMappings || {}), audioSubstitutions: stableValue(inspection.settings.audioSubstitutions || {}), automaticVoiceSubstitutions: inspection.requirements?.automaticVoiceSubstitutions || [], visualOmissions: inspection.omissions || [] }, stats, ownedPaths };
}
...
function validateGbStudioProject({ outputDir, inspection, requireBuild = false } = {}) {
  ...
  const files = new Map(); for (const absolute of walkFiles(root)) { const relative = normalizeRelativePath(path.relative(root, absolute)); if (relative.endsWith('.gbsres') || relative.endsWith('.gbsproj') || relative.startsWith('plugins/') || relative.startsWith('assets/fonts/')) files.set(relative, fs.readFileSync(absolute)); }
