...
test('PCE VN runtime keeps VDC DRAM refresh enabled while toggling display layers', () => {
  ...
  assert.match(source, /static void VN_BANKED_CODE2 preload_scene_assets\(signed int scene_index, uint8_t allow_visual_upload\);/);
  assert.match(source, /static uint8_t adpcm_stream_active = 0;/);
  assert.match(source, /static uint8_t adpcm_stream_looping = 0;/);
  assert.match(source, /static uint8_t adpcm_stream_buffered_fallback = 0;/);
  ...
  assert.match(preloadSceneSource, /if \(\(uint8_t\)scene_index != current_scene\) return;/);
  ...
  assert.match(source, /service_adpcm_streaming\(\);\n    pce_cdb_wait_vblank\(\);\n    service_cdda_playback\(\);/);
