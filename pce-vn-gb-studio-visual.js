...
const { SYSTEM_UI_PALETTE, nearestPaletteIndex, normalizeFourColorPalette } = require('./pce-vn-gb-studio-palette');
const { outputModesForTarget } = require('./pce-vn-gb-studio-target');
...
const TITLE_SELECTOR_ROW_START_Y = 104;
const TITLE_SELECTOR_ROW_SPACING_Y = 16;
...
function isScenarioSelectorScene(scene) {
  return Boolean(scene && (scene.commands || []).some((command) => command?.type === 'spritetext' && command.visible !== false && isScenarioSelectorPrompt(command.text)));
}
...
function spriteTextMapping(command, options = {}) {
  const slot = clampInt(command?.slot, 0, 3, 0); const sourceColor = normalizeRgbHex(command?.color); const titleSceneBlack = Boolean(options.titleScene); const renderedColor = titleSceneBlack ? '#000000' : sourceColor; const rgb = [1, 3, 5].map((at) => parseInt(renderedColor.slice(at, at + 2), 16));
  ...
  const scaledX = Math.round((Number(command?.x) || 0) * 160 / 320); const scaledY = Math.round((Number(command?.y) || 0) * 144 / 224); const selectorLayout = Boolean(options.scenarioSelectorScene) && slot <= 2; const selectorRowY = selectorLayout ? TITLE_SELECTOR_ROW_START_Y + slot * TITLE_SELECTOR_ROW_SPACING_Y : null; const generatedX = Math.max(0, Math.min(160 - 8, scaledX)); const generatedY = Math.max(0, selectorRowY ?? scaledY); const selectorPromptMarginY = titleSceneBlack && isScenarioSelectorPrompt(command?.text) ? generatedY - scaledY : 0;
  const mapping = {
    slot, content: String(command?.text || ''), contentFidelity: 'exact',
    ...
    systemUiPalette: titleSceneBlack,
    policy: { titleSceneBlack, systemUiPalette: titleSceneBlack, selectorLayout, selectorRowY, selectorPromptMarginY, selectorRowSpacingY: selectorLayout ? TITLE_SELECTOR_ROW_SPACING_Y : 0 },
...
function specializeVisualStates(graph, sceneDoc, options = {}) {
  ...
  const ensure = (baseKey, incoming, reachable, fromSceneId = '', sourceSceneEntry = false) => { const base = baseByKey.get(baseKey); if (!base) return null; const entryState = prepareEntry(base, incoming, fromSceneId, sourceSceneEntry); entryState.touch = Math.max(0, ...(entryState.sprites || []).map((sprite) => Number(sprite?.touched) || 0)); if (fromSceneId) cancelMoves(entryState); const entryHash = visualStateHash(serializableVisualState(entryState)); const pair = `${baseKey}\0${entryHash}`; if (byPair.has(pair)) { const existing = byPair.get(pair); existing.reachable ||= reachable; return existing; } const key = `${base.key}::vs_${entryHash.slice(0, 12)}`; const segment = { ...base, key, backgroundStateKey: base.key, visualStateId: entryHash, entryVisualState: serializableVisualState(entryState), exitVisualState: null, visualPlans: [], visualTimelineIds: [], edges: [], targetMap: {}, nextKey: '', reachable: Boolean(reachable) }; const state = clone(entryState);
    const sourceScene = sceneById.get(String(base.sourceSceneId || '')); const visualOptions = { ...options, sourceScene, titleScene: isTitleOrSelectorScene(sourceScene), scenarioSelectorScene: isScenarioSelectorScene(sourceScene) };
...
function drawSpriteText(model, target, spriteText) {
  if (!spriteText?.visible || spriteText.blinkHidden || !spriteText.text) return { glyphs: 0, wrappedLines: 0, uiPaletteTiles: [] };
  const unitId = Object.keys(model.font.assignments || {}).find((id) => id.endsWith(':spritetext') && model.textUnits.find((unit) => unit.id === id)?.text === spriteText.text); const pageIndex = unitId == null ? 0 : model.font.assignments[unitId]; const page = model.font.pages[pageIndex] || model.font.pages[0]; if (!page) return { glyphs: 0, wrappedLines: 0, uiPaletteTiles: [] };
  const atlas = readRgbaPng(page.png); const rgb = /^#?([0-9a-f]{6})$/i.exec(String(spriteText.color || '#ffffff')); const color = rgb ? [parseInt(rgb[1].slice(0, 2), 16), parseInt(rgb[1].slice(2, 4), 16), parseInt(rgb[1].slice(4, 6), 16)] : [255, 255, 255]; const mappedX = Number.isFinite(spriteText.generatedX) ? spriteText.generatedX : Math.round((Number(spriteText.x) || 0) * 160 / 320); const mappedY = Number.isFinite(spriteText.generatedY) ? spriteText.generatedY : Math.round((Number(spriteText.y) || 0) * 144 / 224); let x = Math.max(0, Math.min(target.width - 8, mappedX)); let y = Math.max(0, mappedY); const startX = x; let glyphs = 0; let wrappedLines = 1; const uiPaletteTiles = new Set();
  for (const glyph of Array.from(String(spriteText.text || '').normalize('NFC'))) { if (glyph === '\n') { x = startX; y += 8; wrappedLines += 1; continue; } if (x + 8 > target.width) { x = startX; y += 8; wrappedLines += 1; } if (y + 8 > target.height) { const error = new Error(`SpriteTextが画面下端を超えます: ${spriteText.text}`); error.code = 'GBVN_SPRITETEXT_BOTTOM_OVERFLOW'; throw error; } const code = page.mapping[glyph]; if (code != null) { const tile = code - 32; const sx = (tile % 16) * 8; const sy = Math.floor(tile / 16) * 8; for (let gy = 0; gy < 8; gy += 1) for (let gx = 0; gx < 8; gx += 1) { const sourceAt = ((sy + gy) * atlas.width + sx + gx) * 4; if (luminance([atlas.rgba[sourceAt], atlas.rgba[sourceAt + 1], atlas.rgba[sourceAt + 2]]) < 128) { const dx = x + gx; const dy = y + gy; target.rgba.set([...color, 255], (dy * target.width + dx) * 4); if (spriteText.systemUiPalette) uiPaletteTiles.add(Math.floor(dy / 8) * Math.ceil(target.width / 8) + Math.floor(dx / 8)); } } } x += 8; glyphs += 1; }
  return { glyphs, wrappedLines, uiPaletteTiles: [...uiPaletteTiles].sort((left, right) => left - right) };
}

function composeVisualState(model, baseImage, state, mode, spriteTransforms) {
  const output = { width: baseImage.width, height: baseImage.height, rgba: new Uint8Array(baseImage.rgba) }; if (state?.blank) { for (let index = 0; index < output.width * output.height; index += 1) output.rgba.set([224, 248, 207, 255], index * 4); return { image: output, metrics: { sprites: 0, spriteTextGlyphs: 0, uiPaletteTiles: [] } }; }
  ...
  let glyphs = 0; const uiPaletteTiles = new Set(); for (const spriteText of state?.spriteTexts || []) { const drawn = drawSpriteText(model, output, spriteText); glyphs += drawn.glyphs; drawn.uiPaletteTiles.forEach((tile) => uiPaletteTiles.add(tile)); } return { image: output, metrics: { sprites: spriteCount, spriteTextGlyphs: glyphs, uiPaletteTiles: [...uiPaletteTiles].sort((left, right) => left - right), mode } };
}
...
function quantizeTimelineTile(tile, mode, paletteOverride = null) {
  let palette; if (mode === 'dmg') palette = DMG_COLORS; else if (paletteOverride) palette = normalizeFourColorPalette(paletteOverride); else { const colors = []; for (let index = 0; index < tile.rgba.length; index += 4) colors.push([tile.rgba[index], tile.rgba[index + 1], tile.rgba[index + 2]].map((value) => Math.round(Math.round(value * 31 / 255) * 255 / 31))); palette = normalizeFourColorPalette(medianCutPalette(colors, 4)); }
  const cacheKey = `${mode}:${palette.map((color) => color.join(',')).join(';')}:${sha256(tile.rgba)}`; const cached = timelineTileCache.get(cacheKey); if (cached) return cached; const carrier = image(8, 8); const preview = image(8, 8);
  for (let index = 0; index < 64; index += 1) { const at = index * 4; const color = [tile.rgba[at], tile.rgba[at + 1], tile.rgba[at + 2]]; const chosen = nearestPaletteIndex(color, palette); preview.rgba.set([...palette[chosen], 255], at); carrier.rgba.set([...(mode === 'gbc' ? DMG_COLORS[chosen] : palette[chosen]), 255], at); }
  const result = { carrier, preview }; if (timelineTileCache.size > 100000) timelineTileCache.clear(); timelineTileCache.set(cacheKey, result); return result;
}

function quantizeTimelineFrame(model, background, state, mode) {
  const composed = composeVisualState(model, background.basePrepared, state, mode, model.visual.spriteTransforms || new Map()).image; const converted = image(composed.width, composed.height); const preview = image(composed.width, composed.height);
  for (let ty = 0; ty < 18; ty += 1) for (let tx = 0; tx < 20; tx += 1) { const tileIndex = ty * 20 + tx; const paletteIndex = mode === 'gbc' ? Number(background.gbc?.tilePaletteIndices?.[tileIndex] ?? 0) : 0; const palette = mode === 'gbc' ? (paletteIndex === 7 ? SYSTEM_UI_PALETTE : background.gbc?.palettes?.[paletteIndex]) : null; const tile = quantizeTimelineTile(extractTile(composed, tx, ty), mode, palette); for (let y = 0; y < 8; y += 1) { const destination = ((ty * 8 + y) * converted.width + tx * 8) * 4; converted.rgba.set(tile.carrier.rgba.subarray(y * 32, y * 32 + 32), destination); preview.rgba.set(tile.preview.rgba.subarray(y * 32, y * 32 + 32), destination); } }
  return { source: composed, converted, preview };
}
...
function buildVisualTimelineArtifacts(model, backgrounds, helpers) {
  const outputModes = outputModesForTarget(model.settings.targetMode); const bySegment = new Map(model.graph.segments.map((segment) => [segment.key, segment])); const byBackground = new Map(backgrounds.output.map((entry) => [entry.key, entry])); const timelines = []; const omissions = []; const approximations = []; const loopsByKey = new Map(); const entryLoops = new Map(); const tileBanks = createVisualTileBankAccumulator(helpers);
  if (model.settings.portraitRenderMode === 'actor') for (const sprite of backgrounds.visual?.sprites || []) {
    const timingApproximated = (sprite.timing?.animations || []).some((animation) => (animation.frameErrors || []).some((error) => error !== 0)); const colorApproximated = outputModes.some((mode) => !sprite[mode]?.color?.exact);
  ...
  const ensureLoop = (segment, state) => {
    ...
    for (const config of configs) { const schedule = visualPlaybackSchedule(model, state, config); if (!schedule) return null; if (schedule.overflow) continue; const loopId = `${segment.key}:loop:${stateId.slice(0, 12)}`; const modes = {}; if (schedule.states.length) { for (const mode of outputModes) modes[mode] = loopModeArtifact(model, background, loopId, schedule, mode, helpers); if (outputModes.some((mode) => !modes[mode])) continue; } selected = { loopId, schedule, modes }; break; }
  ...
  for (const spec of model.visual.timelineSpecs || []) {
    ...
    if (usesTiles) for (const mode of outputModes) modes[mode] = timelineModeArtifact(model, background, spec, mode, helpers);
