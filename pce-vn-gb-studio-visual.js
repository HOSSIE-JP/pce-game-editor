...
function spriteTextMapping(command) {
  const sourceColor = normalizeRgbHex(command?.color); const rgb = [1, 3, 5].map((at) => parseInt(sourceColor.slice(at, at + 2), 16));
  ...
  const scaledX = Math.round((Number(command?.x) || 0) * 160 / 320); const scaledY = Math.round((Number(command?.y) || 0) * 144 / 224); const generatedX = Math.max(0, Math.min(160 - 8, scaledX)); const generatedY = Math.max(0, scaledY);
  const mapping = {
    ...
    color: { source: sourceColor, gbc: `#${hexColor(gbcRgb).toLowerCase()}`, dmg: `#${hexColor(dmgRgb).toLowerCase()}` },
    colorFidelity: gbcRgb.every((value, index) => value === rgb[index]) && dmgRgb.every((value, index) => value === rgb[index]) ? 'exact' : 'approximated',
...
function applyVisualCommand(state, command, plan, options = {}) {
  ...
  if (command.type === 'spritetext') { const slot = clampInt(command.slot, 0, 3, 0); plan.action = command.visible === false ? 'text-hide' : 'text-show'; plan.spriteText = spriteTextMapping(command); if ((plan.spriteText.colorFidelity === 'approximated' || plan.spriteText.positionFidelity === 'approximated') && command.visible !== false) plan.fidelity = 'approximated'; if (command.visible === false) state.spriteTexts[slot] = null; else state.spriteTexts[slot] = { text: String(command.text || ''), x: command.x, y: command.y, color: command.color || '#ffffff', blinkFrames: command.blinkFrames || 0, visible: true }; }
...
function specializeVisualStates(graph, sceneDoc, options = {}) {
  ...
  const traverse = (seeds, reachable) => { const queue = [...seeds]; while (queue.length) { const item = queue.shift(); const segment = ensure(item.baseKey, item.state, reachable, item.fromSceneId); if (!segment) continue; const marker = `${segment.key}\0${reachable ? 'r' : 'u'}`; if (processed.has(marker)) continue; processed.add(marker); const base = baseByKey.get(segment.backgroundStateKey); for (const edge of base.edges || []) { const target = ensure(edge.target, segment.exitVisualState, reachable, segment.sourceSceneId); if (!target) continue; segment.targetMap[edge.target] = target.key; if (edge.originTarget) segment.targetMap[edge.originTarget] = target.key; if (!segment.edges.some((candidate) => candidate.target === target.key && candidate.kind === edge.kind)) segment.edges.push({ ...edge, target: target.key }); queue.push({ baseKey: edge.target, state: segment.exitVisualState, fromSceneId: segment.sourceSceneId }); } if (base.nextKey && base.fallthrough) segment.nextKey = segment.targetMap[base.nextKey] || ''; } };
...
function drawSpriteText(model, target, spriteText) {
  ...
  const atlas = readRgbaPng(page.png); const rgb = /^#?([0-9a-f]{6})$/i.exec(String(spriteText.color || '#ffffff')); const color = rgb ? [parseInt(rgb[1].slice(0, 2), 16), parseInt(rgb[1].slice(2, 4), 16), parseInt(rgb[1].slice(4, 6), 16)] : [255, 255, 255]; let x = Math.max(0, Math.min(target.width - 8, Math.round((Number(spriteText.x) || 0) * 160 / 320))); let y = Math.max(0, Math.round((Number(spriteText.y) || 0) * 144 / 224)); const startX = x; let glyphs = 0; let wrappedLines = 1;
...
function buildVisualTimelineArtifacts(model, backgrounds, helpers) {
  ...
  for (const spec of model.visual.timelineSpecs || []) {
    ...
    const usesTiles = model.settings.portraitRenderMode === 'baked' || ['spritetext', 'blank'].includes(spec.kind); const modes = {};
