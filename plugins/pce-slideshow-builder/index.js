...
function onBuildStart(payload, context = {}) {
  ...
  context.logger?.info?.(`PCE HuCard slideshow build start: ${projectDir || '(unknown)'}`);
  return { ok: true };
}
...
function onBuildEnd(payload, context = {}) {
  context.logger?.info?.(`PCE HuCard slideshow ROM generated: ${payload.romPath || '(unknown)'}`);
  return { ok: true };
}

function onBuildError(payload, context = {}) {
  context.logger?.error?.(`PCE HuCard slideshow build failed: ${payload.error || 'unknown error'}`);
  return { ok: true };
}
