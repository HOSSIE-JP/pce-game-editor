...
CD-ROM2 RAM bank の標準ルールは `docs/pce-memory-bank-strategy.md` にまとめています。要点は、bank129 を VN runtime の `VN_BANKED_CODE`、bank130 を 2 本目の `VN_BANKED_CODE2`、bank132 を font / sprite animation / variable 初期値 / scene pack directory などの小さい VN generated data、bank131 を例外的な小さい CPU-readable fallback data に分けることです。script pack と画像/sprite/ADPCM の大きい payload は CD data file のまま扱い、bank129 / bank130 / bank132 へ asset data を混ぜないでください。
