...
`targetMedia: "cd"` の PCE VN build では、generated image の tile data、VRAM 幅へ展開した BG map、sprite pattern、ADPCM 本体を RAM bank へ詰め込まず、project 相対パスのまま `cd.dataFiles` へ登録します。VN script 本体も `assets/generated/vn/scenes/NNN_<sceneId>.bin` の scene pack として `cd.dataFiles` に並べ、bank132 には font、sprite animation、variable 初期値、`pce_vn_scene_packs[]` の sector directory だけを常駐させます。各 scene pack は pointer を持たない little-endian / offset ベース形式で、runtime は scene 入場時に active cache (`4096` bytes) へ読み込みます。明示的な `preload` command が別 scene を走査する場合は preload scan 用の別 cache を使うため、実行中 scene の command/message cache は破壊されません。VN build は各 scene pack をその scene が参照する BG/Sprite/ADPCM data file より前に並べ、build は IPL program の後ろへ padding file を挟み、最初の data file が固定の CD sector 64 から始まるように配置します。runtime は scene 入場時に script pack を読み、ADPCM cache だけを先読みし、BG/Sprite は各表示 command の位置で固定 VRAM 領域へ転送します。明示的な `preload` command は暗転中 (`pending_display_enable`) に限り BG/Sprite の VRAM 先読みを許可し、表示中は ADPCM だけを先読みして画面を変えません。CD-DA と CD data read は同時に行えないため、script pack や画像/sprite/ADPCM の CD data file を読む場合は runtime が CD-DA を `pce_cdb_cdda_pause()` で止めます。CD-DA を維持したい scene では、BG/Sprite command を CD-DA の前に置くか、暗転中の `preload` で必要な表示データを先に詰めてください。ADPCM 読み込みに失敗した場合はロード済みにせず再生もしません。ADPCM は再生中に `preload_scene_assets()` が同じ/別 asset を再 reset/load しないようにし、実際の再生時は必ず `pce_cdb_adpcm_play()` を呼びます。再生前に ADPCM metadata を local snapshot へコピーし、BIOS helper 後に MPR が変わっても length/divider/sector を読み間違えないようにします。`options.stream: true` の ADPCM は preload しません。ADPCM RAM に収まる asset は互換性優先で play 時に buffered 再生し、長尺 asset だけ `pce_cdb_adpcm_stream()` で直接再生します。streaming 中は同じ CD data path を BG/sprite/別 ADPCM 読み込みへ使わない scene 構成にしてください。

ADPCM は llvm-mos SDK の `pce_cdb_adpcm_reset()` / `pce_cdb_adpcm_read_from_cd()` / `pce_cdb_adpcm_read_from_ram()` / `pce_cdb_adpcm_play()` / `pce_cdb_adpcm_stream()` / `pce_cdb_adpcm_stop()` / `pce_cdb_adpcm_status()` 経由で System Card BIOS の ADPCM 機能を使います。通常 asset は CD data file を ADPCM RAM へ読み込んでから再生します。`options.stream: true` の asset でも `min(65535, 65536 - adpcmAddress)` bytes に収まる場合は、互換性を優先して buffered 再生します。ADPCM RAM に収まらない長尺 asset は `ad_stream_start` 相当の CD streaming 経路で直接再生します。streaming 開始直後に stopped へ戻った場合、buffered 可能な asset だけ通常再生へ fallback します。ADPCM BIOS helper は表示状態を変えることがあるため、runtime は表示中に ADPCM load/play/stream を実行した場合だけ完了後に display を再 enable します。暗転中 preload では意図した暗転を維持します。VN の audio command は `pce_cdb_adpcm_play()` / `pce_cdb_adpcm_stream()` を開始したら待ち状態を返さず次の command へ進みます。ただし未 preload の通常 ADPCM と stream 指定 asset の buffered 再生は、再生開始前の ADPCM RAM 読み込みだけ同期的に完了待ちします。
...
CD-ROM2 RAM bank の標準ルールは `docs/pce-memory-bank-strategy.md` にまとめています。要点は、bank129 を VN runtime の `VN_BANKED_CODE`、bank132 を font / sprite animation / variable 初期値 / scene pack directory などの小さい VN generated data、bank130-131 を例外的な小さい CPU-readable fallback data に分けることです。script pack と画像/sprite/ADPCM の大きい payload は CD data file のまま扱い、bank129 / bank132 へ asset data を混ぜないでください。
...
`assets/pce-assets.json` v2 を編集する PC Engine 用の標準アセット管理です。BG image / Sprite sheet / Palette / PSG song/SFX / ADPCM / CD-DA track を扱い、PNG/BMP/WebP は追加時にまずファイル選択ダイアログを開き、選択後にメタ情報を編集してから `image-resize-converter` でリサイズ/クリッピングし、`image-quantize-converter` で 16 色へ減色した後、内蔵 PCE 変換で BG tile / BAT map / Sprite pattern 形式の generated asset を作成します。BMP/WebP は PNG として project 配下へ保存します。変換保存が完了したら戻り値の最新 asset list で一覧を即時更新し、Assets タブを表示したときも自動的に `assets/pce-assets.json` を再読み込みします。WAV/MP3 は追加時にまずファイル選択ダイアログを開き、選択後に `pce-audio-converter` の共通音声 UI で trim、正規化、volume、fade、sample rate、mono/stereo を加工してから ADPCM / CD-DA へ登録します。

---

### `pce-background-manager` / `pce-sprite-manager` — PCE 画像アセット管理
...
| 対応 core | `pc-engine` |
| バージョン | 0.1.0 |
| 依存 | `pce-image-converter` |
| renderer capability | `page`, `background-manager`, `sprite-manager` |

アセット管理とは別の BG image / Sprite sheet 専用エディタです。`pce-background-manager` は `assets/pce-assets.json` v2 の `image` / `kind: "background"` だけを、`pce-sprite-manager` は `sprite` / `kind: "sprite"` だけを一覧表示します。登録済みアセットは一覧から選択して ID / name / palette bank / sprite tile base / sprite position / size / transparent index などのプロパティ編集、削除、画像プレビューを行えます。BG の tile base / map base は asset manager と同じく自動管理され、編集欄には表示しません。

追加時は最初に PNG/BMP/WebP のファイル選択ダイアログを表示し、その後にメタ情報を編集してから `pce-image-converter` の `image-import-pipeline.convertToIndexed16()` を呼びます。最後に `importAssetImage({ kind: "background" | "sprite" })` へ渡し、BG tile / BAT map または sprite pattern の generated asset を作成します。BMP/WebP やリサイズ/減色後の画像は PNG Data URL として保存側へ渡します。
...
| 対応 core | `pc-engine` |
| バージョン | 0.1.0 |
| 依存 | `pce-audio-converter` |
| renderer capability | `page`, `sound-editor`, `adpcm-manager`, `cdda-manager`, `psg-music-editor` |

ADPCM / CD-DA / PSM の音声画面を 1 つの sidebar タブに統合したプラグインです。画面上部のタブで `ADPCM`、`CD-DA`、`PSM` を切り替えます。ADPCM と CD-DA は MP3/WAV の追加、登録済みアセットの編集、削除、音声プレビューを行い、追加時は最初にファイル選択ダイアログを表示してから `pce-audio-converter` の共通音声 UI で project-local WAV を生成します。PSM は `psg-song` / `psg-sfx` の簡易 tracker です。

ADPCM の `divider` は音量ではなく再生速度の rate code です。Sound の ADPCM タブは sample rate から `32000 / (16 - code)` に最も近い `0..15` の code を補完できます。自動分割を有効にした追加では、runtime 側の 16-bit size/address 制約を超える場合に `<id>_part01` 形式の複数 asset として登録されます。

CD-DA タブのリストのドラッグ&ドロップは CD-DA track 順専用で、表示順に `options.track` を 2 始まりの連番へ保存します。PCE-CD build は従来どおり `options.track` 順で CUE の audio track を出力します。

`pce-adpcm-manager` / `pce-cdda-manager` / `pce-music-editor` は互換用の内部モジュール (`hidden: true`) として残し、ユーザー向けプラグイン一覧には表示しません。
...
| タイプ | `editor`, `asset` |
| 対応 core | `pc-engine` |
| renderer capability | `palette-editor` |

アセット管理と同じ `pce-assets.json` を、パレット視点で編集する補助エディタです。スプライト管理は `pce-sprite-manager`、音声編集は `sound-editor` に一本化しています。通常 plugin なので本体 UI へ直書きせず、Plugin Runtime v2.5 の renderer page と capability で mount します。
...
| renderer capability | `page`, `novel-editor`, `visual-novel-editor`, `font-editor` |

VN シーン編集とフォント生成を 1 つの sidebar タブに統合したプラグインです。画面上部のタブで `VN` と `Font` を切り替えます。VN タブは `assets/pce-vn-scenes.json` の scene command を編集し、Font タブは `assets/pce-font.json` を編集して TTF/OTF/TTC フォントから PCE VN 用の 16x16 ビットマップフォントを生成します。フォントプレビューは実際のメッセージ領域と同じ 18文字 x 4行で表示し、保存後に `src/generated/vn.c` のフォントタイルへ反映できます。

`pce-visual-novel-editor` / `pce-font-editor` は互換用の内部モジュール (`hidden: true`) として残し、ユーザー向けプラグイン一覧には表示しません。
...
PCE 用の画像/音声 import を capability として提供します。画像 import pipeline は `convertToIndexed16({ sourcePath, sourceDataUrl, targetSize })` を公開し、PNG/BMP/WebP に対してリサイズ/クリッピング -> 16 色減色 -> PNG Data URL 化のフローを提供します。PCE アセット管理、背景管理、スプライト管理はこの結果を `importAssetImage()` に渡して project-local generated files を作成します。音声は `audio-convert-ui.openAudioConvertModal()` を公開し、WAV/MP3 を Web Audio で読み込み、波形表示、preview、seek、trim、volume、normalize、fade、sample rate、mono/stereo 変換を行った WAV Data URL を返します。実変換は PCE asset API と project-local generated files に集約し、外部コードをアプリ本体へコピーしません。
