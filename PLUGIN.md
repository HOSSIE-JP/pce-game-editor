...
VN build では `src/generated/vn.h` / `vn.c` に `pce_vn_command_t`, `pce_vn_message_t`, `pce_vn_choice_t`, `pce_vn_sprite_anim_t` を出力します。runtime は command を順に実行し、`message`, `choice`, `wait` command で停止します。メッセージ中の入力は typewriter 表示を即時完了し、完了後の入力または `advanceMode: "auto"` の待ち時間経過で次 command へ進みます。`choice` は上下で選択、I/II/RUN で `targetSceneId` へ遷移します。`preload` は暗転中に次 scene の必要 asset を固定 VRAM/ADPCM RAM へ詰め替える用途に限定してください。ADPCM 再生中は preload が ADPCM controller を reset しないため、背景表示などの scene preload が再生中音声を壊しません。CD-DA 再生は explicit な audio command でのみ開始し、`cdda-track.options.track` と同じ track を終端に指定して次 track へ流れないようにします。`cdda-track.options.loop` に応じて `PCE_CDB_CDDA_PLAY_REPEAT` / `PCE_CDB_CDDA_PLAY_ONE_SHOT` を切り替えます。CD-DA 停止は `pce_cdb_cdda_pause()`、ADPCM 停止は `pce_cdb_adpcm_stop()` を使います。
