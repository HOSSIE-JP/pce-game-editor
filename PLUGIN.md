...
`pce-vn-gb-studio-exporter`は`novel-toolbar-action` capabilityとmain hooks `inspectVnGbStudioExport` / `exportVnGbStudioProject` / `validateVnGbStudioProject` / `previewVnGbStudioMusic` / `previewVnGbStudioBackground` / `previewVnGbStudioSprite`を公開します。従来3 hookの名前と入力形式はv1.4.0でも維持します。rendererはfilesystemへ直接書かず、preflight snapshotと設定をmain hookへ渡します。preview hookはproject-local asset IDだけを受理し、source byte・展開寸法・data URL上限を検査します。sprite previewはrender mode、GBC/DMG、使用scene、animation、generation番号を受け取り、同じcropと量子化coreのframe列・OBJ/scanline指標・hashを返します。inspectionは2/3/4択件数、到達性、全raw source commandの消費、sprite catalog、reachable actor pair、visual timeline、OBJ/tile予算、stale設定を返します。core APIは`pce-vn-gb-studio-exporter.js`、CLIも同じcoreとsidecarを使用します。公開targetはGB Studio 4.3.1/4.3.2 / engine `4.3.0-e1`です。
...
出力は任意projectへのmergeではなくmanifest管理のgenerator-owned projectです。v1.4.0はCFGをbackgroundとvisual stateで特殊化し、project全体の`portraitRenderMode`を既定`baked`または`actor`から選びます。baked modeは論理4slot、SpriteText、animation、sync/async moveを最大16 keyframeのtile timelineへ合成し、公式`EVENT_REPLACE_TILE_XY`と隠しcontroller actorで1 frame最大32 tileずつ更新します。actor modeは共通cropの40×48 bust、A/B枠循環、GBC共通3色＋透明、DMG 3階調、flip、animation、公式actor moveを使い、到達可能な2人組の合計40 OBJ・1走査線10 OBJ超過をpreflight errorにします。通常背景はsprite状態を継承し、event stillは消去、blankは論理stateを保持したまま非表示にします。SpriteTextは同じPCE元scene内のGB Studio segment間では継承し、別の元sceneへの遷移および`jump`/`choice`による同scene再入場ではPCE runtimeに合わせて全4slotを消去します。タイトル／シナリオ選択sceneのSpriteTextは黒へ正規化し、「← シナリオ選択 →」だけ変換後Y座標へ8pxを加えてtitle artworkとの間に1tileの余白を確保します。タイトル冒頭の連続SpriteTextは完成状態を背景へ事前焼き込みし、共有blank tileを初期表示時に動的置換しません。非loop animationは1回再生後の最終frameを保持します。`spritetext`内容・座標・色、立ち絵最終状態、本文・分岐・状態・BGMは省略不可です。tile/keyframe削減、blink縮退、時間・色近似は属性単位で`build/qa/visual-audit.json`へ記録し、確認hashが変われば再承認を要求します。sidecar formatはversion 1のまま`portraitRenderMode` / `sprite[assetId]`を加算し、source hash不一致は設定を保持してstale warningにします。旧開発版の`sprites` fieldは読取り時だけ同じ内部mapへ正規化します。native Cやengine overrideには依存しません。制御flow、PSG/背景調整、font、choice pluginの既存契約とPhase 4～6の正本は[docs/pce-vn-gb-studio-exporter.md](docs/pce-vn-gb-studio-exporter.md)です。
