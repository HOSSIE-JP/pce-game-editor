...
CD-ROM2 RAM bankの標準ルールは`docs/pce-memory-bank-strategy.md`を正とします。bank123/MPR6は8192-byte active scene pack、bank128/129/130はresident codeとSystem Card adapter、bank132はgenerated metadata/CD scratch/変換済みglyph cache、bank133はPath B overlay、bank134はSystem Card main/BGM、bank135はsub/SFXです。bank124-127は未使用、bank131はSystem Cardがslot5で使うため使用禁止です。bank121のvisual helper、bank122のCD async/runtime-support helper（palette、部分BAT clear、SATB upload、ADPCM容量判定を含む）、bank104-119のvisual payload cacheは既存配置を維持します。CD VNは`font.bin`/`font_sprite.bin`/旧PSG patternを生成せず、scene pack・画像/sprite/ADPCM・System Card PSG packageをCD data fileとして扱います。link後はbank123/134/135の8KB NOLOAD、console RAM`<=0x1200`、空き`>=2026` bytes、ZP`<=$20E6`、各code bank`<0x2000`に加えてresident bank128/129/130の空き`>=512` bytesをhard gateで検査します。
