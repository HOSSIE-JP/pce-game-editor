...
渡します。画像provenanceはoptionalな`imageTransform`にmanifestの`sourceSize` /
`sourceCrop` / `outputSize`を保持します。warningは明示確認を必須にし、開始後の行失敗・キャンセルでは成功済み
...
ADPCM で `splitPolicy: "auto"` を指定すると、変換後の ADPCM が runtime 側の buffered 安全上限を超える場合に `<id>_part01`, `<id>_part02`, ... の独立 asset として分割登録します。上限は `min(32767, 65536 - adpcmAddress)` bytes です。分割 asset は自動連続再生されないため、scene/message から必要な part を個別に参照してください。
...
検査結果にエラー行があっても、有効行が1件以上あれば実行できます。実行時は行順に変換し、成功行ごとに `assets/pce-assets.json` へ確定してから次へ進みます。失敗行やキャンセル以前の成功は保持されます。戻り値の `results[]` は `lineNumber`, `id`, `status`, `errors[]`, `warnings[]`, `assetIds[]` を持ち、`summary` は成功・失敗・未処理行数と登録 asset / part 数を持ちます。登録 metadata の `data.import.batchFileName` / `batchRow` には CSV basename と行番号だけを保存し、CSV 本体や絶対パスは project へ保存しません。取込後の ADPCM が2048件を超えても登録は続行しますが、CD VN の参照上限を超える警告を返し、build 時の参照数制約は別途適用されます。自動分割された各partもADPCM 1件として数えます。
...
ADPCM の `divider` は再生速度の rate code です。取り込み時は、`divider` 未指定なら `32000 / (16 - code)` が `sampleRate` に最も近い `0..15` の code を自動計算し、代表値は `32000Hz -> 15`, `16000Hz -> 14`, `8000Hz -> 12`, `4000Hz -> 8` です。エディターUIでは現行の入力範囲で実機rate codeに対応する `4000`, `4571`, `5333`, `6400`, `8000`, `10666`, `16000`, `32000` Hz から選択します。`divider` を明示した場合は保存値をそのまま使い、runtime 側でも旧式値としての補正は行いません。buffered playback で安定して鳴らせる 1 asset / part の長さは `min(32767, 65536 - adpcmAddress)` bytes、つまり `bytes * 2 / sampleRate` 秒が目安です。`adpcmAddress: 0` なら 16000Hz で約 4.09 秒、8000Hz で約 8.19 秒です。`assets/generated/<id>/adpcm.bin` は OKI/MSM5205 互換 4-bit adaptive data を高位 nibble 先 (`msn-first`) で保存します。旧 `pce-cd-adpcm-experimental`、古い `lsn-first`、nibble order 未記録、または `encoderVersion` が古い generated file は、source WAV が残っていれば build/source 生成時に自動再生成されます。
ADPCM の true CD streaming (`pce_cdb_adpcm_stream`) は VN runtime / editor 機能から削除しました。ADPCM は常に ADPCM RAM へ読み込んでから System Card BIOS の buffered playback で再生します。長い音声は `splitPolicy: "auto"` で分割する、sample rate を下げる、または CD-DA を使ってください。安全上限を超える ADPCM asset は build error になります。
...
CD VNのlink gateはbank128/129/130の各resident bankと、bank124の`.vn_logic_overlay`に最低1024 bytesの空きを要求します。bank124はMPR4へmapするlogic overlay、bank133はmessage/sprite描画合成overlay、bank121はvisual helper、bank122はCD async/runtime supportとSprite catalog/layout/uploadです。4つのslot4 code blobは同時に見えないため、固定entryとcaller MPR4/MPR6を厳密復元するresident dispatcher以外の相互relocationを作らないでください。

`targetMedia: "cd"`のPCE VN builderが自動登録する管理対象の物理ファイルは、4つのruntime code blobと`vn_payload.bin`です。プロジェクトが明示した追加の`cd.dataFiles`は別にmergeします。CD font payloadは登録しません。logical payloadのstable orderとsector aliasは`vn_payload-index.json`が保持します。bank132には小さいdirectory/ref、CD scratch、`EX_GETFNT`から変換した68-glyph message cacheだけを置き、件数比例するAnimation/PSG metadata配列を常駐させません。
...
各scene packはpointerを持たないlittle-endian / offsetベース形式で、runtimeはscene入場時にbank123のactive cache（8192 bytes）へ読み込みます。明示的な`preload` commandは削除済みです。読み込み最適化はscene入場時の内部preloadが担当します。scene packと参照assetの順序は`vn_payload.bin`内のlogical entry順として固定し、各entryを2048-byte境界へ揃えます。buildはIPL programの後ろへpadding fileを挟み、最初の物理`cd.dataFiles`が固定CD sector 64から始まるように配置します。paddingはELF build後に`pce-mkcd -v`でプログラム像の実sector数を測定し、`PCE_CD_DATA_BASE_SECTOR(64) - program終端sector`で算出します。runtime blobやprogram像のサイズが変わった場合も、pack baseと`vn_payload-index.json`のlogical sector aliasを再計算して実ISO配置と一致させます。

runtime は scene 入場時に script pack を active cache へ読み、暗転中なら最初の待ちコマンドまでに必要な BG/Sprite/ADPCM だけを active cache から先読みし、表示 command では固定 VRAM 領域へ反映します。さらに generator は voice 付き `message` の直前へ内部 `Cache Load ADPCM` を挿入し、手動で同じ ADPCM の cache load が直前にある場合だけ重複を避けます。ボタン待ち message の完了待機中には、次の ADPCM cache load を先取りしません。`background` / `sprite` 表示 command は、VRAM/BAT/SATB 反映、必要な暗黙 fade、表示 layer の再有効化まで完了してから次 command へ進む同期 command です。CD-DA と CD data read は同時に行えないため、script pack や画像/sprite/ADPCM の CD data file を読む場合は runtime が CD-DA を `pce_cdb_cdda_pause()` で止めます。CD-DA を維持したい scene では、BG/Sprite command を CD-DA の前に置いてください。CD-DAはgenerated catalogの絶対開始sectorと、次track先頭（またはlead-out）を指す排他的終了sectorをSystem Cardへ渡します。loopはこの範囲の`REPEAT`、非loopは同じ範囲の`ONE_SHOT`として再生するため、後続trackへ流れません。CD-DA play後はVDC/VCEを再初期化せず、CD/IRQ stateだけを同期します。可視走査中にVCE/R9〜R14を同値で再書込みしても1frameの同期崩れになるためです。ADPCM 読み込みに失敗した場合はロード済みにせず再生もしません。再生前に ADPCM metadata を local snapshot へコピーし、BIOS helper 後に MPR が変わっても length/divider/sector を読み間違えないようにします。Message voice は build 時点で buffered-only に制限され、内部ADPCM cache load完了後、メッセージ窓のVBlank更新・消去・glyph mask準備より前に System Card BIOS の buffered playback を開始します。`pce_cdb_adpcm_play()` へ実際の ADPCM RAM address / `data_size` / divider と `ONE_SHOT` または `REPEAT` を渡し、one-shot の音声終端は hardware の実データ長に任せます。generated `play_frames` はmessage/口パク/AUTOの状態管理だけに使い、自然終了時にstatus polling、追加stop/reset、direct loop restartは行いません。true CD streaming への runtime fallback はありません。長い message voice は `splitPolicy: "auto"` で 32767 byte 以下の part に分ける、sample rate を下げる、または CD-DA へ逃がしてください。

CD-DA と visual payload の補足: 上記の pause は CD data read 区間の制約です。現行既定の `VN_CDDA_RESUME_AFTER_DATA_READ 0` では、raw BG/Sprite payload のCD readを開始した時点でCD-DAを停止扱いとし、自動再開しません。`cd_transfer_scratch` または visual RAM cache から VRAM/BAT へ書き込む間に追加のCD accessは行いません。RAM→VDC data port 転送は HuC6280 の TIA block transfer を使います。PSG 再生中は VRAM 転送を約32 byteごとに区切り、slice ごとに PSG service と MPR 復帰を挟みます。CD settle の補償 tick は PSG 専用で、ADPCM の再生残フレームは実 VBlank credit だけで減らします。PSG が鳴っていない場面では 1 sector までまとめて転送し、full-width BG map は行ごとの 64 byte copy ではなく連続 BAT copy にして、Image command の反映時間を短縮します。BG/Sprite の `cache load` は実験版として低位 System Card RAM へ payload を先読みしますが、VRAM/BAT/SATB へ反映するのは `background` / `sprite` command 実行時だけです。
...
ADPCM は CD 上の payload を bank122 の direct CD/SCSI async helper で ADPCM RAM へ流し込み、32767 bytes 以下の buffered playback は System Card BIOS の `pce_cdb_adpcm_play()` へ実際の read address / `data_size` / divider / `ONE_SHOT` または `REPEAT` を渡して開始します。`0xffff` length の direct latch、frame counterによる早期direct stop、loopのdirect再発行は行いません。安全上限超過は build/import 時に分割または error とし、true CD streaming へ fallback しません。CD から ADPCM RAM へ読む場合、runtime は `vn_wait_next_vblank_raw()` + `engine_service()` + `vn_cd_async_service_frame()` の loading frame で SCSI DATA IN を `IO_PCD_ADPCM_DATA` へ書き、async data phase 中は System Card BIOS helper / external IRQ / `quiet_cd_unit_irqs()` を使いません。message 開始時に cache miss した場合だけ、可視 glyph を描く前に同じ direct async load path で ADPCM RAM へ読み込みます。再生開始をBIOSが受理した後はCD external IRQとpending latchをruntime側でmask/clearし、one-shotの自然終了はgenerated catalogの `play_frames` でruntime状態だけを解放します。自然終了時にADPCM status pollingや追加の `pce_cdb_adpcm_stop()` / `pce_cdb_adpcm_reset()` は行わず、loopはBIOSの `REPEAT` のまま明示停止まで継続します。ADPCM 再生開始後は次の joypad edge 判定を一度だけ初期化します。暗転中 preload では意図した暗転を維持します。VN の audio command は buffered playback を開始したら待ち状態を返さず次の command へ進みます。ただし未 preload の通常 ADPCM は、再生開始前の ADPCM RAM 読み込みだけ同期的に完了待ちします。
...
CD-ROM2 RAM bankの標準ルールは`docs/pce-memory-bank-strategy.md`を正とします。bank123/MPR6は8192-byte active scene pack、bank128/129/130はresident codeとSystem Card adapter、bank132は小さいcatalog directory/ref・CD scratch・変換済みglyph cache、bank124はlogic overlay、bank133はrender/compositor overlay、bank134はSystem Card main/BGM、bank135はsub/SFXです。bank125-127は未使用、bank131はSystem Cardがslot5で使うため使用禁止です。bank121のvisual helper、bank122のCD async/runtime-support helper（palette、部分BAT clear、SATB upload、ADPCM容量判定、Sprite catalog/layout/uploadを含む）、bank104-119のvisual payload cacheは既存配置を維持します。CD VNは`font.bin`/`font_sprite.bin`/旧PSG patternを生成せず、scene pack・画像/sprite/ADPCM・CD on-demand metadata・System Card PSG packageを`vn_payload.bin`の論理payloadとして扱います。link後はbank123/134/135の8KB NOLOAD、console RAM`<=0x1200`、空き`>=2026` bytes、ZP`<=$20E6`、各code bank`<0x2000`に加えてbank128/129/130/124の空き`>=1024` bytesをhard gateで検査します。

PCE background conversion は、入力画像の各 8x8 cell を表示順の tile としてそのまま出力します。同一内容の tile を dedupe しないため、VN の背景切替では絵が過度に共通タイル化されず、raw の `tiles.bin` は `width / 8 * height / 8 * 32` bytes を基準に扱われます。CD-ROM2 でも visual payload は raw の `tiles.bin` / `map_vram.bin` / `patterns.bin` を使い、表示 command 実行時に CD→scratch→VRAM へ chunked 転送します。同一SLOTへ別sprite assetをロードする場合は、runtimeがそのSLOTの旧SATB entryだけを画面外へ逃がしてから、SLOT専用pattern VRAMとpaletteを転送し、完成したSATBを反映します（PCEではゼロSATB entryも実spriteなので、無効化には使いません）。**sprite pattern は background tile と違い、同一内容の表示 cell block を dedupe します。** 変換時に sheet の `cellWidth` × `cellHeight` cell を比較し、ユニークな block だけを `patterns.bin`（= VRAM 転送本体）へ詰めます。16×16 cell は 128 byte の pattern 1 個、32×64 cell は 16×16 pattern 8 個が連続した block になります。各 positional display cell → ユニーク block slot の対応表を `cellmap.bin`（1 byte/cell）として出力します。`generated.tileCount` / `vramBytes` は dedupe 後の 16×16 pattern 数 / byte 数で算出し、`pce_editor_sprite_asset_t.cell_map` に `cellmap.bin` を resident 配列として埋め込みます。runtime の `show_character_sprite_frame()` は positional display cell を `cell_map[]` 経由で VRAM slot へ解決するため、目パチ・口パクなど frame 間で共通する cell block が 1 枚に畳まれ、VN の VRAM 予算（message tile・font mask・SATB を除いた残り）に大きな多 frame sheet を収められます。ユニーク block が 256 を超える sheet は build error（cell map は 1 byte index 上限）です。CD-ROM2 VN catalog は positional display cell も256件までなので、cell列数×行数が256を超えるsheetはbuild errorになります。
...
**各フレームの表示時間（per-frame display time）**: `frameDelay` は全フレーム共通の既定値、`frameDelays`（長さ `frameCount` の配列）は **1 フレームごとの表示フレーム数**です。値は60fps基準の `1..65535` で、`1000` は約16.67秒、`65535` は約18分12.25秒です。スプライトエディタの time フィールド（`spriteEditor.time` = `[[行0…][行1…]]` 行列、1 行 = 1 animation）から保存されます。CD-ROM2では各animationを16-bit per-frame delayを含む256-byte recordとして`sprite_animation_meta.bin`へ出力し、必要なrecordだけをCDから読み、表示中4 slotのruntime cacheへ切り離します。件数比例する`pce_vn_sprite_anim_delays_N[]`やanimation配列を`vn.c`/bank132へ置きません。HuCARDでは従来どおり`pce_vn_sprite_anim_delays_N[]`をgenerated ROM dataへ配置し、`pce_vn_sprite_anim_t.frame_delays`が参照します。両runtimeの16-bit timerは現在frameの値を使い、per-frame値がない場合は`frameDelay`へfallbackします。`frameDelays`を持たない旧assetでも、`spriteEditor.time`行列があれば正規化時にper-frame値へ移行します。timeフィールドは右ペインから直接編集でき、上部のTimeフィールド（ROW/Frame選択）でcell単位に編集できます。

CD-ROM2 VNのlink gateは、bank132/133の空きが256 bytes未満でも物理上限内ならbuildを成功させたまま低headroom警告を出します。bank128/129/130/124はそれより先に1024-byte hard gateで失敗するため、成功buildで256-byte warningだけになることはありません。

CD-ROM2 VN templateはbuilder roleに`pce-visual-novel-builder`を使い、`targetMedia: "cd"`、`toolchain: "llvm-mos"`を使用します。`cd.systemCardProfile`はbuilderが固定値`"jp-v3"`へ正規化する生成契約であり、ユーザー設定ではありません。VN runtimeは`template/template_pce_vn_cd/src/pce_vn_runtime.c`を共通実体とし、build前にcurrent runtime/generated filesを同期します。CD scene pack v3は16-bit Shift-JIS、最大8192 bytesで、旧pack/font/PSG生成物を読む互換layerはありません。version 5 generation stampと必要なmanaged outputが一致する場合は通常BuildでもVN生成を省略し、`skipClean` buildでは最終mediaの入力署名も一致すればcompile/linkを省略します。System Card ROM、PSG driver bytes、抽出glyphは生成物へ含めず、ROM本体は Test Play 時だけSetup設定から検証・使用します。CD-ROM2 project は Export の対象外です。
...
HuCARD VN の bank layout は [docs/pce-vn-hucard-bank-layout.md](docs/pce-vn-hucard-bank-layout.md) を正とします。`rom_bank0` は起動、resident main loop、VDC state、data ref helper、小さい dispatch/metadata だけに残し、runtime worker code は `rom_bank1..4` に固定予約します。bank2 は BG/VRAM と sprite draw/SATB、bank4 は PSG と sprite state/layout/animation/move を担当します。`pce-vn-hucard-manager.js` の `HUCARD_VN_RUNTIME_ROM_BANKS` と `template/template_pce_vn_hucard/src/pce_vn_hucard_banks.h` を同じ配置に保ってください。asset/VN data は `rom_bank5..127 @ slot6` だけを使い、HuCARD VN の `extraDataFiles` は 8KB 未満でも `forceBanked` として `pce_editor_data_ref_t` にします。data-bank allocator は新しいdata refごとにbankを進めず、現在bankの空きへpackし、境界をまたぐrefは複数の`pce_editor_data_chunk_t`へ分割してください。異なるrefが同一bankを共有するため、chunkのbankとリンク済みpointerの両方を正として読みます。HuCARD VN では BG / Sprite の generated visual payload も小さい palette / map を含めて banked data ref にし、bank0 `.rodata` に直置きしません。message font は 12x12 mask、1 glyph = 24 bytes の banked ROM data で、1000 glyph でも bank0 `.rodata` には置きません。scene pack は 4096 bytes cache 上限、spritetext font は別枠 254 glyph 上限を維持し、超過は build error としてください。
  ...
  "scenes": [
    {
      ...
      "commands": [
        ...
        { "type": "message", "speaker": "アカリ", "text": "こんにちは", "textColor": "#ffdb00", "voiceAssetId": "voice_01", "mouthSlot": 0 },
...
VN buildでは`src/generated/vn.h` / `vn.c`にcommand/message/choice/switchの共通型とscene directoryを出力します。HuCARDはさらに`pce_vn_sprite_animations[]`とper-frame delay配列をROM dataとして出力します。CD-ROM2はanimation配列を出力せず、`pce_vn_sprite_animation_meta` directoryと`unsigned int`のcountだけを`vn.c`へ置き、16-bit indexで最大1024件のCD metadata recordを参照します。`-1` sentinelを持つgenerated index fieldは`signed int`です。scene count、start、runtimeのcurrent/preloaded/cache scene indexは`unsigned int`の16-bitで、`VN_MAX_SCENE_COUNT`は32767、無効scene sentinelは`0xffff`です。variable、およびscene pack内でu8 count/indexを使うcommand/message/choice/switchの255件制約は維持します。runtimeはcommandを順に実行し、`message`、`choice`、`wait`で停止します。`background.x` / `background.y`は32x32 BAT上のtile座標で、未指定時は通常BG向けの`(2, 1)`です。`background.transition`は互換用に`"fade"`を保存し、`fadeOutFrames` / `fadeInFrames`は`1 / 20 / 30 / 40 / 50 / 60`へ正規化し、未指定時は速度3の`30`です。値`1`はruntimeで0-frame fadeとして即時切替になり、値`20..60`は指定frame数のpalette fadeです。`sprite` commandは表示・差し替え・非表示を即時反映し、旧`durationFrames` / `moveFrames`は読み込み時に破棄します。
...
message recordは13 byte（`PCE_VN_SCENE_PACK_MESSAGE_SIZE`）です。offset 8のsigned 16-bit `mouth_slot`は`-1`=未指定、`0..3`=対象slot、offset 10のunsigned 8-bit `instant_glyph_count`は話者行など先頭で即時表示するglyph entry数です。`message.mouthSlot`を指定すると、表示中spriteの現在animation ROWから同一asset内の次ROW（index + 1）へ切り替えます。本文表示が完了して入力待機へ入るか、CD one-shot ADPCM voiceが自然終了した時点のうち早い方で元ROWへ自動復帰します。`mouthSlot: null`またはfield省略はナレーション用で口パクしません。最終ROWや別assetへ跨ぐROWは安全にno-opとなり、messageごとの`mouthAnimationId`は使用しません。

`settings.messageSpeedFrames` はノベルエンジン全体のメッセージ速度で、`0 / 10 / 20 / 30 / 40 / 50` のプリセット値へ正規化されます。`settings.messageAdvanceMode` は既定 `"button"` で、`"auto"` の場合は `settings.messageAutoWaitFrames` 経過後に次 command へ進みます。個々の `message` command の旧 `textSpeedFrames` / `advanceMode` / `autoWaitFrames` は読み込み時に破棄され、生成には使われません。`voiceAssetId` に ADPCM を指定した場合、build時にエディタ側が1文字あたりの表示フレームをADPCM実再生長へ合わせます。再生長が取れない場合だけ`settings.messageSpeedFrames`をfallbackとし、runtimeは焼き込まれた値を使います。実再生レートは公称`sampleRate`ではなく量子化レート`32000 / (16 - code)`、同期対象は話者行と改行を除いた本文の発話文字数です。

CD-ROM2 / HuCARD VNは変数index 0を予約名`AUTO_ENABLE`、index 1を`MSG_SPEED`へ固定し、ユーザー変数をindex 2以降へ割り当てます。予約名は大文字・完全一致です。`AUTO_ENABLE`の初期値は`settings.messageAdvanceMode`（button=0 / auto=1）、`MSG_SPEED`は常に0で、scene内の予約名`define`は初期値を変更せず実行時代入になります。Variable / Choiceを含む全書き込みは`AUTO_ENABLE=0..1`、`MSG_SPEED=0..6`へクランプし、IF / Switchは同じ格納値を参照します。`MSG_SPEED=0`はscene packへ焼き込んだ既定／CD音声同期速度、`1..6`は`0 / 10 / 20 / 30 / 40 / 50` frame/文字をメッセージ開始時にスナップショットします。SELECT edgeはInput commandより先に専用消費して`AUTO_ENABLE`を反転するため、Inputの許可ボタンは上下左右・RUN・I・IIです。

AUTOの音声なしMessageは本文完了後に`settings.messageAutoWaitFrames`を待ちます。CD one-shot Message voiceは本文とADPCM自然終了の両方が完了した時点で追加waitなしに進み、自然終了時にstop/resetを発行しません。音声開始失敗とloop voiceはAuto waitへfallbackし、loopは遷移時に明示停止します。独立したAudio commandとHuCARD PSGはAUTO待機条件に含めません。AUTO中も既存の手動全文表示／ページ送りは有効です。生成ヘッダーは予約indexを公開し、変数総数255のうちユーザー変数上限は253です。scene packの13-byte message recordと19-byte command recordは変更しません。

CD-ROM2 / HuCARD の `choice` はカーソルを論理列0、選択肢本文を論理列2から描画します。上下入力ではカーソルの2×2 BATセルだけを同一VBlank内で旧行から新行へ差し替え、選択肢本文の合成済みglyph patternやメッセージ窓全体をクリア・再描画しません。12px glyphが隣接する8px tileを共有していても本文を欠損させず、CD-ROM2でも選択文字列の全体再描画によるちらつきを発生させないruntime契約です。
...
| `pce-vn-gb-studio-exporter` | PCE VN GB Studio出力（移管待ち旧実装） | `converter` | 表示 | v1.5.0。現行実装はstandaloneへ移管済み。parity・公式build/runtime・Portable write監査が完了するまで比較用に一時保持し、完了後に専用core/CLI/test/依存とともに撤去する |
...
> **移管状態:** 現行実装と公開CLIは`C:\homebrew\projects\pce-2-gb-novelgame-converter`を正とします。以下のhookは移管受入が完了するまでの比較対象であり、新規機能を追加しません。汎用`novel-toolbar-action` capabilityは他pluginも使用するため、旧専用実装の撤去後も残します。

`pce-vn-gb-studio-exporter`は`novel-toolbar-action` capabilityとmain hooks `inspectVnGbStudioExport` / `exportVnGbStudioProject` / `validateVnGbStudioProject` / `previewVnGbStudioMusic` / `previewVnGbStudioBackground` / `previewVnGbStudioSprite`を公開します。従来hookの名前と入力形式はv1.5.0でも維持します。rendererはfilesystemへ直接書かず、preflight snapshotと設定をmain hookへ渡します。preview hookはproject-local asset IDだけを受理し、source byte・展開寸法・data URL上限を検査します。sprite previewはrender mode、GBC/DMG、使用scene、animation、generation番号を受け取り、同じcropと量子化coreのframe列・OBJ/scanline指標・hashを返します。inspectionは`targetMode`（`dual` / `gbc` / `gb`）、有効output mode、2/3/4択件数、到達性、全raw source commandの消費、sprite catalog、reachable actor pair、visual timeline、OBJ/tile予算、stale設定を返します。core APIは`pce-vn-gb-studio-exporter.js`、CLIも同じcoreとsidecarを使用します。公開targetはGB Studio 4.3.1/4.3.2 / engine `4.3.0-e1`です。
...
出力は任意projectへのmergeではなくmanifest管理のgenerator-owned projectです。v1.5.0は`targetMode`を`dual`、`gbc`、`gb`から選び、GB Studioの`mixed`、`color`、`mono`へ対応付けます。専用targetでは反対機種のscene・背景・sprite・visual timeline resourceを生成せず、公式build後はCGB flagを順に`0x80`、`0xC0`、`0x00`で検査します。CFGをbackgroundとvisual stateで特殊化し、project全体の`portraitRenderMode`を既定`baked`または`actor`から選びます。baked modeは論理4slot、SpriteText、animation、sync/async moveを最大16 keyframeのtile timelineへ合成し、公式`EVENT_REPLACE_TILE_XY`と隠しcontroller actorで1 frame最大32 tileずつ更新します。actor modeは共通cropの40×48 bust、A/B枠循環、GBC共通3色＋透明、DMG 3階調、flip、animation、公式actor moveを使い、到達可能な2人組の合計40 OBJ・1走査線10 OBJ超過をpreflight errorにします。通常背景はsprite状態を継承し、event stillは消去、blankは論理stateを保持したまま非表示にします。SpriteTextは同じPCE元scene内のGB Studio segment間では継承し、別の元sceneへの遷移および`jump`/`choice`による同scene再入場ではPCE runtimeに合わせて全4slotを消去します。タイトル／シナリオ選択sceneのSpriteTextは黒へ正規化し、slot 0～2をY=104/120/136へ16px間隔で配置します。GBC背景はpalette 0～6を画像用、palette 7の`default-ui`をsystem UI/SpriteText用としてmanual tile paletteへ固定します。タイトル冒頭の連続SpriteTextは完成状態を背景へ事前焼き込みし、共有blank tileを初期表示時に動的置換しません。非loop animationは1回再生後の最終frameを保持します。`spritetext`内容・座標・色、立ち絵最終状態、本文・分岐・状態・BGMは省略不可です。tile/keyframe削減、blink縮退、時間・色近似は属性単位で`build/qa/visual-audit.json`へ記録し、確認hashが変われば再承認を要求します。sidecar formatはversion 1のまま`targetMode` / `portraitRenderMode` / `sprite[assetId]`を加算し、source hash不一致は設定を保持してstale warningにします。旧開発版の`sprites` fieldは読取り時だけ同じ内部mapへ正規化します。native Cやengine overrideには依存しません。制御flow、PSG/背景調整、font、choice pluginの既存契約とPhase 4～6の正本は[docs/pce-vn-gb-studio-exporter.md](docs/pce-vn-gb-studio-exporter.md)です。
`PCE_VN_EVENT_MENU`は折返し後の全label合計を16行までとし、超過は`GBVN_CHOICE_MENU_OVERFLOW`の位置付きerrorで生成前に停止します。
...
inspectは選択rootの`SCRIPT`以下に限定してCP932 SCRを読み、選択SCR、本文を除いた到達可能命令、COLOR token、画像・音声要件、診断、入力signatureを返します。各asset要件にはsourceの拡張子と連結画像間で異なる末尾を除いた`suggestedAssetName`を付け、contextのasset catalogに同名・適合型のassetがあれば`suggestedAssetId` / `suggestedAssetType`も返します。保存済みmappingは選択SCR集合とentryが一致するimport identityだけから復元します。rendererのasset mapping resetは`api.assets.listPceAssets({ force: true })`とinspect hookで最新catalogを再検査し、全asset mappingをsuggestionから再生成しますが、scene所有情報は維持し、sidecarはapply成功時まで書き換えません。話者mapping UIと必須指定はなく、converterは全messageを`speaker: ""`のナレーションとして生成します。`COLOR WIN_MSG`の第2引数は16-bit ARGB4444として静的に解決し、RGBを`message.textColor`へ変換します。解決不能な値は行番号付きwarningにして既定色を使います。`MSG WIN_MSG`は`WAIT WIN_MSG`まで連結し、元SCRの改行とその前後の空白を除去した後、runtimeの17文字折り返しと4行目末尾のcursorを前提に67 glyphずつへ分割します。`IF`はruntime variableが比較の左右どちらにあってもvariableを左へ正規化し、左右交換時は不等号を反転します。`UNLOADCG` / `UNLOAD` / `UNL`はSprite visibilityだけを解除し、同じslotを後続`ICG`で再利用できるようLCG source metadataを保持します。mapping後は`previewConversion: true`、mode、`setStartScene`を指定して非書込みbuild previewを行います。applyはSCR、mapping、mode、disk scene、renderer doc、asset catalog、既存sidecarを再検査し、preview signatureが一致し、全必須asset mappingと`inspectVnSceneDocumentBuild()`を通過した場合だけsceneとsidecarをtemp file経由で保存します。変換前backupは`assets/pce-vn-scenes.kitahe-backup.json`、再取込設定は`assets/kitahe-pm-conversion.json` v1、詳細reportは`assets/kitahe-pm-conversion-report.json`です。sidecarへ絶対source pathと本文を保存してはいけません。画像mappingでSpriteを選ぶ場合はasset ID、slot、animationだけを保存します。各`ICG`のXは、画像assetのoptionalな`kitahePm.imageTransform.sourceCrop.x`を加え、現在のBG幅とtile表示Xを使う`BG表示X + round((ICG X + 元source crop X) * BG幅 / 640)`で変換します。変換後はSpriteの出力幅を含めてBGの左右内側へ補正し、Yを`17`へ固定します。補正時は行番号付きwarningを残します。Background commandは`fadeOutFrames: 30` / `fadeInFrames: 30`（速度3）で生成し、切替演出をPCE VN runtimeのpalette fadeへ任せます。alpha形式の`FADE`がSprite mapping対象なら段階的な透明度ではなく同じslotの`Visible: false/true`へ近似し、`ICG`の初期opacity 0も非表示で生成します。BG対象のalpha `FADE`、9引数の明度系`FADE`、その他のフェードはwarning付きで省略し、`SCREEN`の全画面fade / flash / blank近似だけを維持します。converterはNAME/主人公名置換後の本文と切り詰め後のMENU選択肢をSystem Card jp-v3 encoderでcode point単位に検査し、非対応文字だけを`□`へ置換して`font-character-replaced` warningへ元文字・field・位置・SCR行を残します。この例外はimport sceneだけに限定し、通常のNovel sceneに対するbuild時の厳格な文字errorは変更しません。変換core、mapping、scene所有権、診断仕様は`docs/pce-kitahe-pm-converter.md`を参照してください。

`pce-vn-godot-exporter`は表示名`NVプロジェクトのGodotエクスポート`の組み込みプラグインです。renderer capability `novel-toolbar-action`へ`Godot出力` actionを登録し、ボタンとsnapshot取得・ZIP出力・scene保存の実行順をplugin側で所有します。`novel-editor`は有効な汎用action providerを列挙するだけなので、プラグインOFF時はボタン自体が存在しません。pluginは再利用向けに`vn-godot-exporter.exportPackage({ doc })`も登録します。rendererは汎用`api.plugins.invokeHook()`からmain hook `exportVnGodotPackage`を呼び、専用preload APIや`vn:exportGodotPackage` IPCを追加しません。main hookは`pce-vn-godot-package.js`で`doc`とasset documentを現行形式へ正規化し、SkipされていないCommandが参照するassetだけを収集してstored ZIPを保存します。manifestは`pce-vn-godot-package` version 3、project/entrypoints/visual/audio/stats/filesを持ち、filesには各entryのbyte数とSHA-256を記録します。`data/assets.json`はversion 2で、BG/Spriteの`file`は既定HD file、`visual.hd`はresize/crop確定後・PCE減色前のproject-local PNG、`visual.pce`は最終`palette.bin`とBG tileまたはSprite pattern/cellmapから再構成したindexed PNGを指します。標準image importはHD PNGを`assets/images-hd/`または`assets/sprites-hd/`へ保存し、`data.import.highQualitySource`にproject相対pathを記録します。旧assetにHD sourceがなければ現行`asset.source`をfallbackとして収録し、`stats.visualHighQualityFallbackAssets`とplugin結果に件数を返します。visual/audio sourceはproject外pathを拒否します。WAVは`@audio/encode-ogg`のOgg Vorbis encoderでVBR quality 4へ変換し、既存OGG/MP3は再encodeしません。保存先を確認してから変換を開始し、画像再構成または音声変換失敗時はZIPを書きません。`entrypoints.font`は`readFontConfig(projectDir).fontPath`で選択中のproject fontだけを指し、未選択font libraryの先頭fileへfallbackしません。Godot Playerの外枠画像はエディタprojectへ同梱せず、Player配布側の`package/library.json`トップレベル`border`から`package`相対pathで読み込みます。Godot package manifestの`entrypoints.border`は空文字です。project IDは既存`id`/`projectId`/`uuid`を優先し、ない場合はproject directoryとmetadataから安定生成します。詳細なpackage/runtime契約は`docs/pce-vn-godot-exporter.md`を正とします。

`inspectKitahePmSource` / `applyKitahePmConversion` の主人公名は「ハドソン」が既定です。inspectは同じ選択SCR集合・entryのsidecarに保存名があれば`protagonistName`として返し、rendererで明示変更した値を優先します。`【主人公】` / `主人公`などの既知tokenはSCRの`NAME`再定義より指定主人公名を優先し、本文とMENU選択肢の両方へ適用します。
