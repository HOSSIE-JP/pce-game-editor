...
出力は任意projectへのmergeではなくmanifest管理のgenerator-owned projectです。`Color + Monochrome`設定、GBC/DMG二重scene graph、GBC 7 background palettes + UI reserved palette、DMG固定4色/192 tile gate、font page、MODと曲別auditをまとめて生成します。各font pageはPNGと`.png.gbsres`に加え、GB Studio compilerがmappingを読む同名`.json`を必須生成し、三者のmappingと本文のinline font参照を静的検査します。本文またはChoiceの直前はGB Studio標準`EVENT_SET_DIALOGUE_FRAME`で既定frame tileを再転送し、project-local native pluginへ依存しません。targetなしasync Inputと後続sync Inputで構成するselectorは全経路をscene-local callbackへ変換し、`EVENT_IDLE`で入力待ちします。CD-DA mappingは`psg-song`または検証済み4ch ProTracker MOD、message voiceは話者別text toneです。sidecar、manifest、diagnostic code、将来Phaseの正本は[docs/pce-vn-gb-studio-exporter.md](docs/pce-vn-gb-studio-exporter.md)です。
