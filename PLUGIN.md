...
`targetMedia: "cd"` の PCE VN build では、generated image の tile data、VRAM 幅へ展開した BG map、sprite pattern、ADPCM 本体を RAM bank へ詰め込まず、project 相対パスのまま `cd.dataFiles` へ登録します。VN build は scene command から必要 asset を解決し、scene pack 順を優先して `cd.dataFiles` を並べます。build は IPL program の後ろへ padding file を挟み、最初の visual/audio data file が固定の CD sector 64 から始まるように配置します。runtime は scene 入場時に ADPCM cache だけを先読みし、BG/Sprite は各表示 command の位置で固定 VRAM 領域へ転送します。明示的な `preload` command は暗転中 (`pending_display_enable`) に限り BG/Sprite の VRAM 先読みを許可し、表示中は ADPCM だけを先読みして画面を変えません。現状の VN command/message/scene tables は scene 単位に CD から動的ロードする形式ではなく、bank132 の generated data として常駐します。CD-DA と CD data read は同時に行えないため、再生中に画像/sprite/ADPCM の CD data file を読む場合は runtime が CD-DA を `pce_cdb_cdda_pause()` で止めます。CD-DA を維持したい scene では、BG/Sprite command を CD-DA の前に置くか、暗転中の `preload` で必要な表示データを先に詰めてください。ADPCM 読み込みに失敗した場合はロード済みにせず再生もしません。ADPCM は再生中に `preload_scene_assets()` が同じ/別 asset を再 reset/load しないようにし、実際の再生時は必ず `pce_cdb_adpcm_play()` を呼びます。再生前に ADPCM metadata を local snapshot へコピーし、BIOS helper 後に MPR が変わっても length/divider/sector を読み間違えないようにします。`options.stream: true` の ADPCM は preload せず、play 時に CD data file を `pce_cdb_adpcm_stream()` で直接再生します。streaming 中は同じ CD data path を BG/sprite/別 ADPCM 読み込みへ使わない scene 構成にしてください。
...
PCE アセット管理とは別の BG image / Sprite sheet 専用エディタです。`pce-background-manager` は `assets/pce-assets.json` v2 の `image` / `kind: "background"` だけを、`pce-sprite-manager` は `sprite` / `kind: "sprite"` だけを一覧表示します。登録済みアセットは一覧から選択して ID / name / palette bank / sprite tile base / sprite position / size / transparent index などのプロパティ編集、削除、画像プレビューを行えます。BG の tile base / map base は asset manager と同じく自動管理され、編集欄には表示しません。
...
### `pce-adpcm-manager` — PCE ADPCM 管理
...
| renderer capability | `page`, `adpcm-manager` |

PCE アセット管理とは別の ADPCM 専用エディタです。`assets/pce-assets.json` v2 の `adpcm` だけを一覧表示し、MP3/WAV の追加、登録済みアセットの ID / name / sample rate / ADPCM RAM address / divider / loop / streaming 編集、削除、音声プレビューを行います。追加時は最初にファイル選択ダイアログを表示し、その後 `pce-audio-converter` の共通音声 UI で ADPCM 用 WAV を生成して `importAssetAudio({ kind: "adpcm", splitPolicy: "auto" })` に渡します。

ADPCM の `divider` は音量ではなく再生速度です。`pce-adpcm-manager` は sample rate から `round(32000 / sampleRate - 1)` で divider を補完できます。自動分割を有効にした追加では、runtime 側の 16-bit size/address 制約を超える場合に `<id>_part01` 形式の複数 asset として登録されます。

---

### `pce-cdda-manager` — PCE CD-DA 管理
...
| バージョン | 0.1.0 |
| 依存 | `pce-audio-converter` |
| renderer capability | `page`, `cdda-manager` |

PCE アセット管理とは別の CD-DA 専用エディタです。`assets/pce-assets.json` v2 の `cdda-track` だけを一覧表示し、MP3/WAV の追加、登録済みトラックの編集、削除、音声プレビューを行います。追加時は最初にファイル選択ダイアログを表示し、その後 `pce-audio-converter` の共通音声 UI で ISO 用 CD-DA WAV を生成して `importAssetAudio({ kind: "cdda-track" })` に渡します。

リストのドラッグ&ドロップは CD-DA track 順専用で、表示順に `options.track` を 2 始まりの連番へ保存します。PCE-CD build は従来どおり `options.track` 順で CUE の audio track を出力します。

---

### `pce-palette-editor` / `pce-music-editor`

| 項目 | 値 |
|---|---|
| タイプ | `editor`, `asset` |
| 対応 core | `pc-engine` |
| renderer capability | `palette-editor`, `psg-music-editor` |

PCE アセット管理と同じ `pce-assets.json` を、パレット・PSG トラッカーの各視点で編集する補助エディタです。スプライト管理は `pce-sprite-manager` に一本化しています。通常 plugin なので本体 UI へ直書きせず、Plugin Runtime v2.5 の renderer page と capability で mount します。
