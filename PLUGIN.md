...
PCE アセット管理とは別の ADPCM 専用エディタです。`assets/pce-assets.json` v2 の `adpcm` だけを一覧表示し、MP3/WAV の追加、登録済みアセットの ID / name / sample rate / ADPCM RAM address / divider / loop 編集、削除、音声プレビューを行います。追加時は最初にファイル選択ダイアログを表示し、その後 `pce-audio-converter` の共通音声 UI で ADPCM 用 WAV を生成して `importAssetAudio({ kind: "adpcm", splitPolicy: "auto" })` に渡します。
