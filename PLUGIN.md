...
| `pce-vn-gb-studio-exporter` | PCE VN GB Studio出力 | `converter` | 表示 | v1.3.0。Novel toolbarからGB Studio 4.3.1/4.3.2 mixed-mode project、背景状態CFG、PSG/画像個別調整、制御flow監査、任意の公式ROM/Web buildを生成。有効時だけ`GB Studio出力`を表示 |
...
`pce-vn-gb-studio-exporter`は`novel-toolbar-action` capabilityとmain hooks `inspectVnGbStudioExport` / `exportVnGbStudioProject` / `validateVnGbStudioProject` / `previewVnGbStudioMusic` / `previewVnGbStudioBackground`を公開します。従来3 hookの名前と入力形式はv1.3.0でも維持します。rendererはfilesystemへ直接書かず、preflight snapshotと設定をmain hookへ渡します。preview hookはproject-local asset IDだけを受理し、背景PNGのbyte・展開寸法・data URL上限を検査します。inspectionは2/3/4択件数、背景状態付きblock到達性、全raw source commandの消費状況、generated pluginとcustom event参照、BGM/背景調整catalogを返します。core APIは`pce-vn-gb-studio-exporter.js`のinspection/generate/validation/preview関数で、CLIも同じcoreとsidecarを使用します。公開targetはGB Studio 4.3.1/4.3.2 / engine `4.3.0-e1`です。実行file pathは外側の引用符を除去してから`app.asar`を検査します。
...
出力は任意projectへのmergeではなくmanifest管理のgenerator-owned projectです。`Color + Monochrome`設定、GBC/DMG二重scene graph、GBC 7 background palettes + UI reserved palette、DMG固定4色/192 tile gate、font page、MODと曲別auditをまとめて生成します。comment/cacheは後続label/executable blockへ付属させ、実行命令を持たない断片を到達不能warningにしません。PCEの持続背景はCFGの`block × inheritedBackground`状態として伝播し、異なる背景が背景なしblockへ合流する場合はsceneを背景別に特殊化します。正規化前の全commandを`sceneId + commandIndex`で棚卸しし、GBC/DMG event ID、処理区分、到達性、`originBlockKey`、entry/effective background、特殊化scene IDを`build/qa/control-flow-audit.json`へ保存します。未分類、未確認省略、分岐・状態・本文・BGMの欠落はerrorです。PSGはperiodを音高の第一情報として6chの活動量・音量・長さ・wave/noiseを4 roleへ自動割当し、flat/sharp正規化、cent誤差、音色置換、競合/dropを曲別監査します。export modalから曲別tempoとchannel target/instrument/volume/transpose/priority、画像別brightness/saturation/GBC/DMG ditherを調整でき、200ms debounceとgeneration番号で古いpreviewを破棄します。WebAudioは近似previewであり、正式判定は公式ROMです。各font pageはPNGと`.png.gbsres`に加え、GB Studio compilerがmappingを読む同名`.json`を必須生成し、三者のmappingと本文のinline font参照を静的検査します。2～4択またはrandomを使うprojectだけへ公式Script Event Plugin機構の`plugins/pce-vn-control`を同梱し、native Cやengine overrideへ依存しません。sidecar formatはversion 1のまま`music` / `backgrounds`を加算し、正式export成功時だけ保存します。Phase 3のsprite/spritemove/fade/flash/blankは凍結中です。sidecar、manifest、diagnostic code、Phase 3～6の正本は[docs/pce-vn-gb-studio-exporter.md](docs/pce-vn-gb-studio-exporter.md)です。
