...
ADPCM の `divider` は音量ではなく再生速度の rate code です。`pce-adpcm-manager` は sample rate から `32000 / (16 - code)` に最も近い `0..15` の code を補完できます。自動分割を有効にした追加では、runtime 側の 16-bit size/address 制約を超える場合に `<id>_part01` 形式の複数 asset として登録されます。
