...
`pce-kitahe-pm-converter`の選択SCRは、entry SCRからexternal GOTOで到達できる集合だけへ絞りません。entryの連結graphを先に解析し、未到達の選択SCRを先頭から独立rootとして順次解析するため、接続済みSCRの状態継承を維持しながら全選択SCRをscene生成対象にします。entry指定は変換後の`entrySceneId`を決めます。解析上のbasic blockは同じSCR内でbuild時command見積り220件／scene pack見積り7000 bytesを目標にpackingし、同一sceneの遷移はentry labelへの`goto`へ変換します。Choiceは選択値を保存した直後の`switch`でlocal labelまたはbridge `jump`へ振り分け、Choice分岐だけを理由にsceneを分けません。初回inspectは`summary.basicBlockCount`と独立rootの`summary.minimumSceneCount`を返し、minimumがPCE VN runtime上限255を超える場合だけ`scene-count-limit` errorにします。mapping後はpacking済みscene数とVN managerの実255 command／8192-byte pack検査を通し、一部だけの暗黙取込は行いません。
